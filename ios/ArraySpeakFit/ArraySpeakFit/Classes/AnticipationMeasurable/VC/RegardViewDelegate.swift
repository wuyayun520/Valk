...
fileprivate let k_quoteRecordValue:[UInt8] = [0xb,0xc,0xb,0x16,0x4a,0x1,0xd,0x6,0x7,0x10,0x58,0x4b,0x42,0xa,0x3,0x11,0x42,0xc,0xd,0x16,0x42,0x0,0x7,0x7,0xc,0x42,0xb,0xf,0x12,0xe,0x7,0xf,0x7,0xc,0x16,0x7,0x6]

private func sleepingDraught(appeal num: UInt8) -> UInt8 {
    return num ^ 98
}

/*: "Post" :*/
fileprivate let app_listenerValue:[Character] = ["P","o","s","t"]

/*: "Profile" :*/
fileprivate let notiChipTitle:String = "Profildisc constitute hat exposure"
fileprivate let dreamDistanceIdent:String = "add"

/*: "bth_me_follow_nor" :*/
fileprivate let main_shrinkReaderCapacityName:String = "visual part busy varbth_me"
fileprivate let showImportantIdent:String = "ow_nordemonstrate silly inmate map kingdom"

/*: "party_mini_bg_userDetail" :*/
fileprivate let kRepresentationExpressBeautifulFormat:[Character] = ["p","a","r","t","y","_","m","i","n","i","_","b","g","_","u","s","e","r","D","e","t","a"]
fileprivate let user_materialName:String = "productl"

/*: "removeAttentionUid" :*/
fileprivate let user_pendingRefUnableData:String = "rewhyv"
fileprivate let showMiddlePath:[Character] = ["t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let k_themStr:[Character] = ["a","t","t","e","n","t","i","o","n","U","i","d"]

/*: "bth_me_follow_pre" :*/
fileprivate let m_secondRemainUrl:[UInt8] = [0x49,0x5f,0x43,0x74,0x46,0x4e,0x74,0x4d,0x44,0x47,0x47,0x44,0x5c,0x74,0x5b,0x59,0x4e]

private func connectionShe(protection num: UInt8) -> UInt8 {
    return num ^ 43
}
...
class RegardViewDelegate: FictionalCharacterRecognizerDelegate {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_quoteRecordValue.map{sleepingDraught(appeal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: CommentView = { // 导航
    ...
    private lazy var bottomView: SocialView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = SocialView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: dreamNoSampleValue, width: kGiftIdent, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.appLayer()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.telecom()
        ...
        indicator.indicatorColor = UIColor.telecom()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(kSoundPath)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(app_listenerValue)).localized, (String(notiChipTitle.prefix(6)) + dreamDistanceIdent.replacingOccurrences(of: "add", with: "e")).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [FictionalCharacterRecognizerDelegate] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: EveryViewController = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = EveryViewController()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: ProfileViewDelegate = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = ProfileViewDelegate()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(main_shrinkReaderCapacityName.suffix(6)) + "_foll" + String(showImportantIdent.prefix(6))))
    ...
    private lazy var voiceRoomBtn: UIButton = {
        ...
        voiceRoomBtn.setBackgroundImage(UIImage.verbalDescription(name: (String(kRepresentationExpressBeautifulFormat) + user_materialName.replacingOccurrences(of: "product", with: "i"))), for: .normal)
        //: return voiceRoomBtn
        return voiceRoomBtn
        //: }()
    }()
...
extension RegardViewDelegate {
    ...
    private func message() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(user_pendingRefUnableData.replacingOccurrences(of: "why", with: "mo") + "eAtten" + String(showMiddlePath))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(String(k_themStr))] = userInfoModel.uid
        }
    ...
    func keep() {
        ...
        queue.async(group: group) {
            ...
            FurtheranceReactiveCompatible.receiveCompletion(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.who()
                //: if self.uid == AppManager.share.loginUid {
                if self.uid == AppManager.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.subedit()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<BagHandyJSON>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.attach(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.mindSet(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.no(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.eyeValidController(userModel)
                        ...
                        self.roomHeadImgView.beyondFore(urlStr: userModel.headPic, options: optionsInfo)
                        // 场景三：男性用户邀请通话弹窗监听
                        //: (TalkingApplication.shared as! TalkingApplication).showMaleInviteCallView_2(uid: userModel.uid, headPic: userModel.headPic)
                        (ApplicationReactiveCompatible.shared as! ApplicationReactiveCompatible).currencyInviteBirthday(uid: userModel.uid, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            FurtheranceReactiveCompatible.allow(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = AnticipationMeasurable.deserialize(from: dic as? Dictionary) {
    ...
    private func bag() {
        ...
        var profileSectionArr = [UsufructuaryCaseIterable.profileType]
        ...
        self.profileVC.observer(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension RegardViewDelegate {
    ...
    private func dismissStatus() {
        ...
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: m_secondRemainUrl.map{connectionShe(protection: $0)}, encoding: .utf8)! : (String(main_shrinkReaderCapacityName.suffix(6)) + "_foll" + String(showImportantIdent.prefix(6)))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.verbalDescription(name: followImgStr)
    }

    /// 展示VIP皮肤视图
    //: private func setupVIPSkinView() {
    private func cutLibrary() {
        ...
        vipCenterView.simultaneouslyElsewhere(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.simultaneouslyElsewhere(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension RegardViewDelegate: JXSegmentedViewDelegate {
...
extension RegardViewDelegate: JXPagingViewDelegate {
...
extension RegardViewDelegate {
    ...
    private func who() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(notiFilterStr)
        }
    }
    ...
    private func icon() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: kGiftIdent, height: user_allMessage)
        ...
        voiceRoomBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-actualHeight(h: 178) - user_sizeValue)
    ...
    private func direct() {
        ...
        voiceRoomBtn.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                //: guard let self = self else {return}
                guard let self = self else { return }
                //: PipSqueakReactiveCompatible.share.func_pushToVoiceRoomVC(roomId: self.userInfoModel.partyRoomId.stringValue)
                PipSqueakReactiveCompatible.share.doingeVoice(roomId: self.userInfoModel.partyRoomId.stringValue)
                //: })
            })
