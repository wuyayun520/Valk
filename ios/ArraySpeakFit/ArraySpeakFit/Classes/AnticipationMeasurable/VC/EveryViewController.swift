...
fileprivate let noti_submitHandStr:[Character] = ["i","c","o","n","_"]
fileprivate let m_certainFormat:[Character] = ["k","o","n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let mCostValue:[UInt8] = [0xa3,0xb2,0xb4,0xb6]

private func documentRating(listen num: UInt8) -> UInt8 {
    return num ^ 211
}

/*: "uid" :*/
fileprivate let dreamColonShadowUrl:[UInt8] = [0xce,0xd2,0xdf]

private func missMagnitude(input num: UInt8) -> UInt8 {
    return num ^ 187
}

/*: "list" :*/
fileprivate let dream_struggleName:String = "llengthst"
...
class EveryViewController: FictionalCharacterRecognizerDelegate {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [SayTransformable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: notiFilterStr, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(NobodyItemCell.self, forCellWithReuseIdentifier: NobodyItemCell.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(noti_submitHandStr) + String(m_certainFormat)))
...
extension EveryViewController {
    //: private func headerRefresh() {
    private func networkBright() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        sessionShow()
    ...
    private func sessionShow() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: mCostValue.map{documentRating(listen: $0)}, encoding: .utf8)!: pageIndex, String(bytes: dreamColonShadowUrl.map{missMagnitude(input: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        BubbleReactiveCompatible.close(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(dream_struggleName.replacingOccurrences(of: "length", with: "i"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = ComprehensiveAlterTransformable.deserialize(from: dict as? Dictionary) {
...
extension EveryViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NobodyItemCell.className(), for: indexPath) as! NobodyItemCell
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        var detailModel = ComprehensiveAlterTransformable()
        ...
        let vc = SwiftDetailVc(model: detailModel)
...
extension EveryViewController: JXPagingViewListViewDelegate {
...
extension EveryViewController {
    ...
    func eyeValidController(_ userModel: BagHandyJSON) {
