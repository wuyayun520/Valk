...
fileprivate let userEveningMsg:[UInt8] = [0x2d,0x2a,0x2d,0x30,0x6c,0x27,0x2b,0x20,0x21,0x36,0x7e,0x6d,0x64,0x2c,0x25,0x37,0x64,0x2a,0x2b,0x30,0x64,0x26,0x21,0x21,0x2a,0x64,0x2d,0x29,0x34,0x28,0x21,0x29,0x21,0x2a,0x30,0x21,0x20]

private func enjoyPolitic(existing num: UInt8) -> UInt8 {
    return num ^ 68
}

/*: "icon_me_voice_white" :*/
fileprivate let dreamNestText:String = "speaker daily liveicon_"
fileprivate let main_clientStr:String = "circle lack successfulme_voi"

/*: "btn_auto_write" :*/
fileprivate let dream_primaryPath:String = "btn_auhere export"
fileprivate let dream_mysteryKey:[Character] = ["i","t","e"]

/*: "Pending review" :*/
fileprivate let mainEnabledMessage:String = "Penparent obstacle"
fileprivate let notiChooseContactTopMsg:String = "well adjustmentding r"

/*: "get json error" :*/
fileprivate let noti_reverseTitle:String = "root same process simply representget j"
fileprivate let dream_operateName:String = "son devote statement about letter"
...
class MatterView: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        selectionStyle = UITableViewCell.SelectionStyle.none
        //: self.setupSubviews()
        self.micConTranslation()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userEveningMsg.map{enjoyPolitic(existing: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var audioBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.gravitySize(colors: UIColor.decelerate(), size: CGSize(width: 155, height: 35)), for: .normal)
        //: btn.layer.cornerRadius = 17.5
        btn.layer.cornerRadius = 17.5
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(recordBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(sinceOwn), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var playIconView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.verbalDescription(name: (String(dreamNestText.suffix(5)) + String(main_clientStr.suffix(6)) + "ce_white")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var editBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(dream_primaryPath.prefix(6)) + "to_wr" + String(dream_mysteryKey))), for: .normal)
        //: btn.addTarget(self, action: #selector(editBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(each), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension MatterView {
    ...
    func between(model: BubbleHandyJSON, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(mainEnabledMessage.prefix(3)) + String(notiChooseContactTopMsg.suffix(6)) + "eview").localized
    ...
    func shadowEffect() {
        ...
        do {
            ...
            let url = EffectTool.default.variationType(type: .Play_Audio_Wave)
            ...
            printLog(message: (String(noti_reverseTitle.suffix(5)) + String(dream_operateName.prefix(4)) + "error"))
        }
    }
}
...
extension MatterView {
    ...
    private func micConTranslation() {
