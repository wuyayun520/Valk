...
fileprivate let appMaleMsg:[UInt8] = [0x96,0x91,0x96,0x8b,0xd7,0x9c,0x90,0x9b,0x9a,0x8d,0xc5,0xd6,0xdf,0x97,0x9e,0x8c,0xdf,0x91,0x90,0x8b,0xdf,0x9d,0x9a,0x9a,0x91,0xdf,0x96,0x92,0x8f,0x93,0x9a,0x92,0x9a,0x91,0x8b,0x9a,0x9b]

/*: "More" :*/
fileprivate let mMinimumData:String = "Morespend added version maybe"

/*: "icon_details_arrow" :*/
fileprivate let main_cloudKey:[Character] = ["i","c","o","n","_","d","e","t"]
fileprivate let show_priorityHeatEarlyIdent:[Character] = ["a","i","l","s","_","a","r","r","o","w"]

/*: "Post" :*/
fileprivate let notiConversionUrl:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let app_rejectStr:[Character] = ["b","t","n","_","d","y","n","a","m","i","c","_","s","t","o","p","_"]
fileprivate let user_translateIdent:String = "noflow"
...
class SeeReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appMaleMsg.map{$0^255}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .optimize(0, 0, 0, 0.4)
    ...
    lazy var moreButton: GoddamnThen = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .concern(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(mMinimumData.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(main_cloudKey) + String(show_priorityHeatEarlyIdent))), for: .normal)
...
extension SeeReactiveCompatible {
    ...
    func map(userModel: BagHandyJSON) {
        ...
        strA = [(String(notiConversionUrl)).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.telecom(), UIColor.componentHighlight()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.equitySize(fontSize: 18), UIFont.equitySize(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: SayTransformable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.damage())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    electronicNetwork(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.damage())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    electronicNetwork(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.damage())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    electronicNetwork(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func snap() {
        ...
        let vc = CoatButtonViewController(uid: userId)
    ...
    func electronicNetwork(btn: UIImageView) {
        //: btn.subviews.map {
        btn.subviews.map {
            //: (view) -> () in
            view in
        ...
        let imag = UIImageView(image: UIImage(named: (String(app_rejectStr) + user_translateIdent.replacingOccurrences(of: "flow", with: "r"))))
...
extension SeeReactiveCompatible {
    ...
    private func build() {
        ...
        let height = (kGiftIdent - actualWidth(w: 15 * 4)) / 3.0
