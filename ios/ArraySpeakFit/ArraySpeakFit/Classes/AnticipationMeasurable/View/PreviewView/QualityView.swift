...
fileprivate let app_cryVacationFormat:[UInt8] = [0x6c,0x6b,0x6c,0x71,0x2d,0x66,0x6a,0x61,0x60,0x77,0x3f,0x2c,0x25,0x6d,0x64,0x76,0x25,0x6b,0x6a,0x71,0x25,0x67,0x60,0x60,0x6b,0x25,0x6c,0x68,0x75,0x69,0x60,0x68,0x60,0x6b,0x71,0x60,0x61]

private func sharpDistance(upon num: UInt8) -> UInt8 {
    return num ^ 5
}

/*: "img_loading_lose" :*/
fileprivate let show_lightUrl:String = "img_lconsent of outer tension export"
fileprivate let app_temporaryContent:String = "g_loseupon enjoy"
...
class QualityView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_cryVacationFormat.map{sharpDistance(upon: $0)}, encoding: .utf8)!)
    }
...
extension QualityView {
    ...
    private func cur() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(before))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(gallery)).then {
...
extension QualityView {
    ...
    func chase(imgModel: ValueAttemptGalleryModel) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(show_lightUrl.prefix(5)) + "oadin" + String(app_temporaryContent.prefix(6)))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.minimumSubviews()
            }
        }
    ...
    func character(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(show_lightUrl.prefix(5)) + "oadin" + String(app_temporaryContent.prefix(6)))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.minimumSubviews()
        }
    }
...
extension QualityView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension QualityView {
    ...
    func minimumSubviews() {
        ...
        var newH = kGiftIdent * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > user_allMessage {
            //: newH = ScreenHeight
            newH = user_allMessage
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * user_allMessage / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (kGiftIdent - newW) / 2.0, y: (user_allMessage - newH) / 2)
