...
fileprivate let k_structureTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_me_edit" :*/
fileprivate let show_towardPath:String = "loose deliverybtn_m"
fileprivate let show_ticketTitle:[Character] = ["e"]
fileprivate let m_tailValue:String = "stroke stack toward appearance last_edit"
...
class RowViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_structureTitle.reversed(), encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        if index == 0 && islast {
            ...
            backView.shRadii(width: backView.frame.size.width, height: backView.frame.size.height, corners: .allCorners, cornerRadii: .init(width: 8, height: 8))
            //: } else if index == 0 {
        } else if index == 0 {
            ...
            backView.shRadii(width: backView.frame.size.width, height: backView.frame.size.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 8, height: 8))
            //: } else if islast {
        } else if islast {
            ...
            backView.shRadii(width: backView.frame.size.width, height: backView.frame.size.height, corners: [.bottomLeft, .bottomRight], cornerRadii: .init(width: 8, height: 8))
            //: } else {
        } else {
            ...
            backView.shRadii(width: backView.frame.size.width, height: backView.frame.size.height, corners: .allCorners, cornerRadii: .init(width: 0, height: 0))
        }
    }
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.verbalDescription(name: (String(show_towardPath.suffix(5)) + String(show_ticketTitle) + String(m_tailValue.suffix(5)))))
...
extension RowViewCell {
    //: func setCellMessage(titile: String, row: Int, isLast: Bool) {
    func work(titile: String, row: Int, isLast: Bool) {
    ...
    func game() {
        ...
        titleBLB.font = UIFont.concern(type: .Medium, fontSize: 15)
        //: detailLB.font = UIFont.pingfangFont(type: .Medium, fontSize: 15)
        detailLB.font = UIFont.concern(type: .Medium, fontSize: 15)
        //: editImag.isHidden = false
        editImag.isHidden = false
