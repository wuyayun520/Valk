...
fileprivate let mainWowData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let mCreatureFormat:[Character] = ["E","d","i","t"," ","T"]
fileprivate let notiEarlyText:String = "itsea"

/*: "Finish" :*/
fileprivate let user_extraStr:[Character] = ["F","i","n","i","s","h"]

/*: "btn_delete" :*/
fileprivate let m_simplyName:[Character] = ["b","t","n","_","d","e"]
fileprivate let kMatterId:String = "lfalltfall"
...
class FacialNerveTextView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(actionView))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainWowData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(mCreatureFormat) + notiEarlyText.replacingOccurrences(of: "sea", with: "le")).localized
    ...
    lazy var inputTView: DumpScrollView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(user_extraStr)).localized, for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(m_simplyName) + kMatterId.replacingOccurrences(of: "fall", with: "e"))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(image), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension FacialNerveTextView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = clearDetail(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func clearDetail(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
