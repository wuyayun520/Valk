...
fileprivate let k_exitContent:[UInt8] = [0x76,0x7b,0x76,0x81,0x35,0x70,0x7c,0x71,0x72,0x7f,0x47,0x36,0x2d,0x75,0x6e,0x80,0x2d,0x7b,0x7c,0x81,0x2d,0x6f,0x72,0x72,0x7b,0x2d,0x76,0x7a,0x7d,0x79,0x72,0x7a,0x72,0x7b,0x81,0x72,0x71]

fileprivate func donTone(log num: UInt8) -> UInt8 {
...
/*: "Birthday" :*/
fileprivate let notiByPath:String = "resource found shouldBirth"
fileprivate let dreamHeatSecretPath:String = "dengineeringy"

/*: "btn_me_edit" :*/
fileprivate let user_tearId:String = "class vanishbtn_"
...
// __CLOSE_PRINT__
//
//  GuideStuffObjectProtocol.swift
//  AbroadTalking
//
...
class GuideStuffObjectProtocol: UITableViewCell, RelatedViewDelegate {
    //: var myDateView: TalkingDateView!
    var myDateView: AlongsideGraceReactiveCompatible!
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_exitContent.map{donTone(log: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(notiByPath.suffix(5)) + dreamHeatSecretPath.replacingOccurrences(of: "engineering", with: "a")).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.verbalDescription(name: (String(user_tearId.suffix(4)) + "me_edit")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        dataBtn.setTitleColor(UIColor.telecom(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.concern(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(click), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
...
extension GuideStuffObjectProtocol {
    //: @objc func dataBtnClick() {
    @objc func click() {
        //: PipSqueakReactiveCompatible.share.func__getCurrentActivityVC()?.view.endEditing(true)
        PipSqueakReactiveCompatible.share.funcProcess()?.view.endEditing(true)
        //: myDateView = TalkingDateView.init()
        myDateView = AlongsideGraceReactiveCompatible()
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.pendingHinduCalendarMonth(year: year, month: month, day: day)
            }
        }
    }
