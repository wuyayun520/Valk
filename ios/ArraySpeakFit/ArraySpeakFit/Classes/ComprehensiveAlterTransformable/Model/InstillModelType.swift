...
/*: "%@ reply %@：%@" :*/
fileprivate let user_reKey:String = "%@ replytar cold valid attention outer"
fileprivate let mainLapValue:String = " %@：%@"
...
struct InstillModelType: HandyJSON {
    //: var cid: String?
...
extension InstillModelType {
    ...
    mutating func sumer() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(user_reKey.prefix(8)) + mainLapValue.capitalized), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(SeeThen.commonality(text: tempContent, size: CGSize(width: kGiftIdent - 82, height: CGFloat(MAXFLOAT)), font: UIFont.concern(type: .Regular, fontSize: 14), lineNumber: 0).height)
