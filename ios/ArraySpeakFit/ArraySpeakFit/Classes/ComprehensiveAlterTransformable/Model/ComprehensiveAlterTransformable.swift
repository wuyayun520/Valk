...
// __CLOSE_PRINT__
//
//  ComprehensiveAlterTransformable.swift
//  AbroadTalking
//
...
struct ComprehensiveAlterTransformable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct TransportModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum MomentOfTruthQualityAbbreviationValueConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct FlagMeasurable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: BucketOutputStreamable?
}

///
//: enum MomentVideoStatus: String {
enum BucketOutputStreamable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ComprehensiveAlterTransformable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func perPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(SeeThen.commonality(text: content!, size: CGSize(width: kGiftIdent - 54, height: CGFloat(MAXFLOAT)), font: UIFont.concern(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MomentOfTruthQualityAbbreviationValueConvertible.Photo.rawValue || momentType == MomentOfTruthQualityAbbreviationValueConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
