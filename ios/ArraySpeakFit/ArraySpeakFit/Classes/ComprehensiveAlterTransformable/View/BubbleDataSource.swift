...
fileprivate let userHimTeeKey:[UInt8] = [0x70,0x77,0x70,0x6d,0x31,0x7a,0x76,0x7d,0x7c,0x6b,0x23,0x30,0x39,0x71,0x78,0x6a,0x39,0x77,0x76,0x6d,0x39,0x7b,0x7c,0x7c,0x77,0x39,0x70,0x74,0x69,0x75,0x7c,0x74,0x7c,0x77,0x6d,0x7c,0x7d]
...
class BubbleDataSource: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userHimTeeKey.map{$0^25}, encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func flow(model: ComprehensiveAlterTransformable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension BubbleDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: kGiftIdent - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (kGiftIdent - 30 - 24 - 30) / 3 : (kGiftIdent - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: kGiftIdent, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: CommunicateView = collectionView.dequeueReusableCell(withReuseIdentifier: CommunicateView.className(), for: indexPath) as! CommunicateView
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: TransportModelType = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.slideQuantityerrupt(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = MatterFlowLayout(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension BubbleDataSource {
    // 添加视图
    //: private func setupSubviews() {
    private func nevertheless() {
        //: backgroundColor = .clear
        backgroundColor = .clear
        //: register(TalkingMomentPhotosCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosCell.className())
        register(CommunicateView.self, forCellWithReuseIdentifier: CommunicateView.className())
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
