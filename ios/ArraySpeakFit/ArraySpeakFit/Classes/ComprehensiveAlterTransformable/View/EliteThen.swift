...
fileprivate let notiGoId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_home_v" :*/
fileprivate let k_eventStr:String = "hangcon"

/*: "icon_lounge" :*/
fileprivate let user_projectionValue:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "#AAAAAA" :*/
fileprivate let main_dropFormat:String = "#AAAAAAsun engage edit superior data"

/*: "icon_moment_nor_report" :*/
fileprivate let k_marginContent:[Character] = ["i","c","o","n","_","m","o","m","e","n","t","_","n","o","r","_"]
fileprivate let noti_democraticId:String = "thenepothent"

/*: "img_me_edit_photo_default" :*/
fileprivate let dream_publisherKey:[Character] = ["i","m","g","_","m","e","_","e","d","i","t"]
fileprivate let noti_enterFormat:[Character] = ["_","p","h","o","t","o"]
fileprivate let mainSoulLastMessage:String = "note client fatal_default"

/*: "icon_home_boy" :*/
fileprivate let appDonorText:String = "exclusive opposite contenticon_ho"
fileprivate let mainHugeBadMsg:String = "too presentation screenme_boy"

/*: "icon_home_girl" :*/
fileprivate let mainPeerSharpThemeName:String = "icon_hofamily approval transform last support"
fileprivate let appTeeStr:[Character] = ["m","e","_","g","i","r","l"]

/*: "level_" :*/
fileprivate let userExpectedPath:[Character] = ["l","e","v","e","l","_"]
...
class EliteThen: UITableViewCell {
    //: var cid = ""
    var cid = ""
    //: var uid = ""
    var uid = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiGoId.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(dealSh), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
    ...
    lazy var cardImgView: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (k_eventStr.replacingOccurrences(of: "hang", with: "i") + "_home_v"))
    ...
    lazy var sexBtn: UIButton = {
        ...
        btn.titleLabel?.font = .concern(type: .Medium, fontSize: 11)
        //: self.addSubview(btn)
        self.addSubview(btn)
        //: return btn
        return btn
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(user_projectionValue)))
        //: self.addSubview(img)
        self.addSubview(img)
        //: return img
        return img
    ...
    lazy var timeLB: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(main_dropFormat.prefix(7))))
        //: self.addSubview(label)
        self.addSubview(label)
        //: return label
        return label
    ...
    lazy var BlockBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(k_marginContent) + noti_democraticId.replacingOccurrences(of: "then", with: "r"))), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
...
extension EliteThen {
    ...
    @objc func want(gestureRecongnizer: UIGestureRecognizer) {
    ...
    @objc func block() {
        ...
        let reportView = TransportThen(frame: UIScreen.main.bounds, type: .reportCommentType, rId: cid)
        //: reportView.showReportViewIn(view: ImmediatelyThen.getWindow())
        reportView.asRemove(view: ImmediatelyThen.wow())
    }
}
...
extension EliteThen {
    ...
    func customerOn(model: ParentModelType) {
        //: cid = model.cid ?? ""
        cid = model.cid ?? ""
        //: uid = model.uid ?? ""
        uid = model.uid ?? ""
        //: iconBtn.setUrlImage(urlStr: model.headPic ?? "", placeImg: UIImage.BundleImageNamed(name: "img_me_edit_photo_default"))
        iconBtn.attentionStateSoil(urlStr: model.headPic ?? "", placeImg: UIImage.verbalDescription(name: (String(dream_publisherKey) + String(noti_enterFormat) + String(mainSoulLastMessage.suffix(8)))))
        ...
        nameLabel.textColor = model.loungePlus ? .atSituationRaw() : .componentHighlight()
        ...
        case AlongsideDigitalTableConvertible.male.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_boy"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.verbalDescription(name: (String(appDonorText.suffix(7)) + String(mainHugeBadMsg.suffix(6)))), for: .normal)
        //: break
        //: case Gender.female.rawValue:
        case AlongsideDigitalTableConvertible.female.rawValue:
            //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: "icon_home_girl"), for: .normal)
            sexBtn.setBackgroundImage(UIImage.verbalDescription(name: (String(mainPeerSharpThemeName.prefix(7)) + String(appTeeStr))), for: .normal)
        //: break
        //: default:
        default:
            //: break
        ...
        levelBtn.setBackgroundImage(UIImage.verbalDescription(name: (String(userExpectedPath)) + String(model.level ?? 1)), for: .normal)
...
extension EliteThen {
    ...
    private func visual() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(briquette))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let longPress = UILongPressGestureRecognizer(target: self, action: #selector(onLongTapCell(gestureRecongnizer:)))
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(want(gestureRecongnizer:)))
