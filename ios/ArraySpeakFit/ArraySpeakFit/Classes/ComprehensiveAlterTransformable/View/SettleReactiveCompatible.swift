...
fileprivate let main_bodyFormat:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

/*: "Are you sure you want to delete the video?" :*/
fileprivate let userReinUrl:[UInt8] = [0xd9,0xea,0xfd,0xb8,0xe1,0xf7,0xed,0xb8,0xeb,0xed,0xea,0xfd,0xb8,0xe1,0xf7,0xed,0xb8,0xef,0xf9,0xf6,0xec,0xb8,0xec,0xf7,0xb8,0xfc,0xfd,0xf4,0xfd,0xec,0xfd,0xb8,0xec,0xf0,0xfd,0xb8,0xee,0xf1,0xfc,0xfd,0xf7,0xa7]

private func discHat(mutual num: UInt8) -> UInt8 {
    return num ^ 152
}

/*: "Cancel" :*/
fileprivate let m_lastMsg:[Character] = ["C","a","n","c","e","l"]

/*: "btn_moment_pic_add" :*/
fileprivate let show_powerName:[Character] = ["b","t","n","_","m","o","m","e","n","t","_","p","i","c","_","a","d"]
fileprivate let main_lostWholeDownPath:String = "incident"
...
protocol AgencyViewDelegate: NSObjectProtocol {
    /// 添加图片
...
class SettleReactiveCompatible: UICollectionView {
    // 最大选中图片数
    ...
    weak var delegte: AgencyViewDelegate?
    //: var headerView: FreeCollectionHeaderView?
    var headerView: BridgeViewDelegate?
    //: var contentType: MomentType?
    var contentType: MomentOfTruthQualityAbbreviationValueConvertible?

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_bodyFormat.map{$0^132}, encoding: .utf8)!)
    }
    ...
    override init(frame: CGRect, collectionViewLayout _: UICollectionViewLayout) {
        ...
        flowLayout.sectionInset = UIEdgeInsets(top: 0,
                                               ...
                                               right: kGiftIdent - mainSoundName * 3 - 15 - 7 * 2)
        //: flowLayout.minimumLineSpacing = 7
        flowLayout.minimumLineSpacing = 7
        //: flowLayout.minimumInteritemSpacing = 7
        flowLayout.minimumInteritemSpacing = 7
        //: flowLayout.headerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 240+22))
        flowLayout.headerReferenceSize = CGSize(width: kGiftIdent, height: actualWidth(w: 240 + 22))
        //: flowLayout.footerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 56))
        flowLayout.footerReferenceSize = CGSize(width: kGiftIdent, height: actualWidth(w: 56))
        //: super.init(frame: frame, collectionViewLayout: flowLayout)
        super.init(frame: frame, collectionViewLayout: flowLayout)
        //: createUI()
        spatialPropertyDetail()
    }

    // MARK: - UI

    //: private func createUI() {
    private func spatialPropertyDetail() {
        ...
        self.register(SickbagViewCell.self, forCellWithReuseIdentifier: SickbagViewCell.className())
        ...
        self.register(BridgeViewDelegate.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: BridgeViewDelegate.className())
        ...
        self.register(CommentFooterView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: CommentFooterView.className())
        //: self.delegate = self
        self.delegate = self
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(switcher(gestureRecongnizer:)))
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(adpressedSearch))
...
extension SettleReactiveCompatible: UIGestureRecognizerDelegate {
    //: @objc func closeKeyBoard() {
    @objc func adpressedSearch() {
        //: self.endEditing(true)
        self.endEditing(true)
    }

    //: @objc func longPressMethod(gestureRecongnizer: UILongPressGestureRecognizer) {
    @objc func switcher(gestureRecongnizer: UILongPressGestureRecognizer) {
    ...
    func pointEnable(point _: CGPoint) {}

    /// 正在拖拽
    //: func dragChange(point: CGPoint) {
    func mainDoing(point _: CGPoint) {}

    /// 结束拖拽
    //: func dragEnd(point: CGPoint) {
    func memoryMinimum(point _: CGPoint) {}
}
...
extension SettleReactiveCompatible: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: SickbagViewCell.className(), for: indexPath) as! SickbagViewCell
        //: cell.deleteBlock = {
            ...
            if indexPath.row < self.imageArray.count {
                ...
                if self.contentType == .Video {
                    ...
                    DemonstrateSayAlertReactiveCompatible.rightUsed(title: nil,
                                               ...
                                               message: String(bytes: userReinUrl.map{discHat(mutual: $0)}, encoding: .utf8)!.localized,
                                               //: leftBtnTitle: "Cancel".localized,
                                               leftBtnTitle: (String(m_lastMsg)).localized,
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加按钮
            ...
            cell.releaseImageView.image = UIImage.verbalDescription(name: (String(show_powerName) + main_lostWholeDownPath.replacingOccurrences(of: "incident", with: "d")))
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            headerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: BridgeViewDelegate.className(), for: indexPath) as? BridgeViewDelegate
            //: return headerView!
            return headerView!
            //: } else {
        } else {
            ...
            let footerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: CommentFooterView.className(), for: indexPath) as! CommentFooterView
            //: return footerView
            return footerView
        }
    }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加图片
            ...
            if contentType == .Video {
                ...
                self.delegte?.goodLooks(index: indexPath)
            }
        }
    }
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if contentType == .Video {
            ...
            return CGSize(width: mainSoundName, height: mainSoundName)
        }
    }
}
