...
fileprivate let userGraceStr:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let user_qualityTitle:String = "btn_score background correct"
fileprivate let app_restoreKey:[Character] = ["c","_"]
fileprivate let notiKeepMsg:String = "eh former mutual loosestop_nor"

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let showButtonPath:[Character] = ["b","t","n","w","O","b","l","E","h","_"]
fileprivate let m_administratorReadMsg:String = "bill short sharp mediatdj_o"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let dreamSuccessfullyFaultMsg:String = "finite bybtnCmj"
fileprivate let userTimingIdent:String = "ointoi"
...
protocol AbsenceThen: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func makeRejectStatus(view: DemandReactiveCompatible, status: LicenseViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func picture(view: DemandReactiveCompatible, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class DemandReactiveCompatible: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: AbsenceThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userGraceStr.map{$0^223}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(user_qualityTitle.prefix(4)) + "dynami" + String(app_restoreKey) + String(notiKeepMsg.suffix(8)))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(income(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(showButtonPath) + String(m_administratorReadMsg.suffix(5)) + "ediv_stop")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.verbalDescription(name: (String(dreamSuccessfullyFaultMsg.suffix(6)) + "oS7_tdj_" + userTimingIdent.replacingOccurrences(of: "into", with: "ed") + "v_play")), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(playOrientationFastenerMoveAction(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.verbalDescription(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: NoteThen = {
        ...
        let slid = NoteThen(frame: CGRect(x: 100, y: 0, width: kGiftIdent - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension DemandReactiveCompatible {
    ...
    private func utilised() {
        ...
        self.slider.panBeginBlock = { [weak self] in
            ...
            self.haveStatus(status: .StartSeek)
        }
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.picture(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.haveStatus(status: .StopSeek)
        }
    }
}
