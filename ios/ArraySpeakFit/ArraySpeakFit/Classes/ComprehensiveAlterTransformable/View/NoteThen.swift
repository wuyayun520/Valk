...
fileprivate let mRecordingHoneyName:[UInt8] = [0x35,0x32,0x35,0x28,0x74,0x3f,0x33,0x38,0x39,0x2e,0x66,0x75,0x7c,0x34,0x3d,0x2f,0x7c,0x32,0x33,0x28,0x7c,0x3e,0x39,0x39,0x32,0x7c,0x35,0x31,0x2c,0x30,0x39,0x31,0x39,0x32,0x28,0x39,0x38]

private func himStorage(stream num: UInt8) -> UInt8 {
    return num ^ 92
}
...
class NoteThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mRecordingHoneyName.map{himStorage(stream: $0)}, encoding: .utf8)!)
    }
    ...
    init(frame: CGRect, sliderWidth: CGFloat, sliderColor: UIColor, progressHeight: CGFloat, progressBgColor: UIColor, progressPlayedColor: UIColor, progressCachedColor: UIColor, isShowCorner: Bool) {
        ...
        self.meanSolarDayTotaleraction()
    }
    ...
    private lazy var progressView: WorldViewThen = {
        ...
        let v = WorldViewThen(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(rid(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(tapDailyJab(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension NoteThen {
    ...
    @objc func rid(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func tapDailyJab(gesture: UIPanGestureRecognizer) {
...
extension NoteThen {
    ...
    private func meanSolarDayTotaleraction() {}
}
