...
fileprivate let k_patterPath:[UInt8] = [0x7c,0x81,0x7c,0x87,0x3b,0x76,0x82,0x77,0x78,0x85,0x4d,0x3c,0x33,0x7b,0x74,0x86,0x33,0x81,0x82,0x87,0x33,0x75,0x78,0x78,0x81,0x33,0x7c,0x80,0x83,0x7f,0x78,0x80,0x78,0x81,0x87,0x78,0x77]

fileprivate func protectionEnough(temp num: UInt8) -> UInt8 {
...
class AlterView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_patterPath.map{protectionEnough(temp: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func oldIn() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(tinkle))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(resThrough))
...
extension AlterView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func tinkle(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func resThrough(tap: UIGestureRecognizer) {
    ...
    func mic(url: String) {
        ...
        imageView.simultaneouslyElsewhere(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func sum(image: UIImage) {
