...
fileprivate let main_complaintMessage:[UInt8] = [0x2f,0x34,0x2f,0x3a,0xee,0x29,0x35,0x2a,0x2b,0x38,0x0,0xef,0xe6,0x2e,0x27,0x39,0xe6,0x34,0x35,0x3a,0xe6,0x28,0x2b,0x2b,0x34,0xe6,0x2f,0x33,0x36,0x32,0x2b,0x33,0x2b,0x34,0x3a,0x2b,0x2a]

fileprivate func billLine(estimated num: UInt8) -> UInt8 {
...
/*: "#4FAAFF" :*/
fileprivate let app_reamId:String = "narrow visible#4FAAFF"

/*: "reply" :*/
fileprivate let showTinPath:String = "replbeauty"

/*: "F5F5F5" :*/
fileprivate let dreamRemainingIdent:String = "Ftuntun5"
...
// __CLOSE_PRINT__
//
//  CharacterReactiveCompatible.swift
//  AbroadTalking
//
...
class CharacterReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_complaintMessage.map{billLine(estimated: $0)}, encoding: .utf8)!)
    }
...
extension CharacterReactiveCompatible {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func beforeMissDrag(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension CharacterReactiveCompatible {
    //: func configCell(model: TalkingCommentReplyModel) {
    func turnDown(model: InstillModelType) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(app_reamId.suffix(7))))
            ...
            let font = UIFont.duringWindowExtreme(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (showTinPath.replacingOccurrences(of: "beauty", with: "y")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.appLayer(), UIColor.telecom(), UIColor.appLayer(), UIColor.telecom()]
                ...
                colorA = [UIColor.appLayer(), UIColor.telecom()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
            ...
            attributedString = attributedString.mpBy(texts: strA, colors: colorA, fonts: fontA)
    ...
    func clickFresh(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kGiftIdent - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (dreamRemainingIdent.replacingOccurrences(of: "tun", with: "5F")))?.cgColor
...
extension CharacterReactiveCompatible {
    ...
    private func off() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(beforeMissDrag(sender:)))
