...
/*: "tab" :*/
fileprivate let showBullUrl:String = "installb"

/*: "page" :*/
fileprivate let mLineStr:String = "PAGE"

/*: "uid" :*/
fileprivate let notiDemonstrateKey:String = "pathid"

/*: "model" :*/
fileprivate let dream_automaticallyPath:String = "modminimuml"

/*: "There's no feed yet." :*/
fileprivate let k_temporaryId:String = "situation elite gray bottom moreTher"
fileprivate let notiSiblingClearName:String = "e\'s n"
fileprivate let main_exploreUrl:String = "d yet.opportunity moment"
...
class CommunicateViewDelegate: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var dataSourceArr: [TalkingMomentModel] = []
    var dataSourceArr: [ComprehensiveAlterTransformable] = []

    //: var seleteCell = TalkingMomentItemCell()
    var seleteCell = EditViewCell()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: dreamOffValue, right: 0)
        //: return table
        return table
...
extension CommunicateViewDelegate {
    //: func reqData() {
    func sinceData() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(showBullUrl.replacingOccurrences(of: "install", with: "ta"))] = 1
        //: dict["page"] = pageIndex
        dict[(mLineStr.lowercased())] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        BubbleReactiveCompatible.ta(params: dict) { succeed, result, errorModel in
            ...
            if succeed {
                ...
                var dataArr: [ComprehensiveAlterTransformable] = []
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<ComprehensiveAlterTransformable>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
...
extension CommunicateViewDelegate {
    ...
    @objc func languageIn(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(notiDemonstrateKey.replacingOccurrences(of: "path", with: "u"))] as! String
    ...
    @objc func hour(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ComprehensiveAlterTransformable = userinfo[(dream_automaticallyPath.replacingOccurrences(of: "minimum", with: "e"))] as! ComprehensiveAlterTransformable
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func drownOf(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ComprehensiveAlterTransformable = userinfo[(dream_automaticallyPath.replacingOccurrences(of: "minimum", with: "e"))] as! ComprehensiveAlterTransformable
...
extension CommunicateViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = EditViewCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? EditViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = EditViewCell(style: .default, reuseIdentifier: identifier)
        }
        //: let model: TalkingMomentModel = self.dataSourceArr[indexPath.row]
        let model: ComprehensiveAlterTransformable = self.dataSourceArr[indexPath.row]
        //: cell?.isListTableCell = true
        cell?.isListTableCell = true
        //: cell?.configCell(model: model)
        cell?.hebdomadModel(model: model)
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == MomentOfTruthQualityAbbreviationValueConvertible.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: EditViewCell = cell as? EditViewCell else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
                    tempcell.telecastingInput()
                }
            }
        }
    }
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = SwiftDetailVc(model: model)
...
extension CommunicateViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        pullOff(scrollView: scrollView)
    }
    ...
    func pullOff(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: EditViewCell = mainTableView.cellForRow(at: middleIndexPath) as! EditViewCell
                ...
                if model.momentType == MomentOfTruthQualityAbbreviationValueConvertible.Video.rawValue {
                    ...
                    if lastVideoUrl != model.videoInfo?.videoUrl {
                        // 在播放下一个视频前停止当前视频
                        //: seleteCell.stopPlay()
                        seleteCell.makeRepresentation()
                        //: cell.stopPlay()
                        cell.makeRepresentation()
...
extension CommunicateViewDelegate {
    ...
    private func state() {
        ...
        style.TipsTitle = (String(k_temporaryId.suffix(4)) + notiSiblingClearName + "o fee" + String(main_exploreUrl.prefix(6))).localized
        ...
        mainTableView.register(EditViewCell.self, forCellReuseIdentifier: EditViewCell.className())
    ...
    private func appear() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(hour(notification:)), name: user_closeNearbyPopFormat, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(languageIn(notification:)), name: app_dismissMessage, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(drownOf(notification:)), name: userThirdId, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(align), name: userSenseStr, object: nil)
    }
}
