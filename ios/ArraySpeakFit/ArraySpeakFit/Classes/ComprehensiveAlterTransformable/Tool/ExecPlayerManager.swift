...
/*: "/TXCache" :*/
fileprivate let mainSumerruptFormat:String = "track yellow tail solid/TXCache"
...
enum TelecastingBridgePortPlayerStatus {
    //: case Unknown
    case Unknown // 未知
    //: case ReadyToPlay
...
protocol RowThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func cryForStatus(player: ExecPlayerManager, status: TelecastingBridgePortPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func skin(player: ExecPlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func noPress(player: ExecPlayerManager, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class ExecPlayerManager: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: RowThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: TelecastingBridgePortPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = ExecPlayerManager()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(mainSumerruptFormat.suffix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(molarConcentration), userInfo: nil, repeats: true)
...
extension ExecPlayerManager {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func satisfyPer(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: mainLastName.doGender(), with: k_filterName)
    ...
    @objc func molarConcentration() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.noPress(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func mp(renderMode: MatterRenderMode) {
    ...
    func size(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isObject == true {
        if StackSocketManager.shared.isObject == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func spaceDisable() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.cryForStatus(player: self, status: self.playStatus!)
        }
    }
...
extension ExecPlayerManager: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.skin(player: self, duration: Int(duration), currentTime: Int(currentTime))
