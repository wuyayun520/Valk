...
class PerformanceTool: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: OthersChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = ButtonBoltThen()          // 视频渲染配置类
    ...
    private var trtcView: CryView?    // 预览view（使用商汤美颜）
...
extension PerformanceTool {
    ...
    func converge(_ frontCamera: Bool) {
        ...
        self.needCalendar(front: frontCamera)
    }
    ...
    func bedSource() {
        ...
        DispatchQueue.main.async {
            ...
            self.chief(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension PerformanceTool {
    ...
    private func needCalendar(front: Bool) {
        ...
        self.trtcView?.serverLoad(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func siphon(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard main_versionFormat == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.accept(isOpen: isOpen)
...
extension PerformanceTool {
    ...
    func counterrupt() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(betweenUs(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(tradeRouteNotif(notif:)),
    ...
    @objc private func pan(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func betweenUs(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc func tradeRouteNotif(notif: Notification) {
...
extension PerformanceTool {
    ...
    func chief(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = CryView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.addAbove(isSmallWindow: smallWindow)
        //: if smallWindow { 
...
//    }
    ...
    private func diskListen() {
        ...
        if main_versionFormat {
            ...
            self.trtcView?.stretchUpwardlyShortAndSweetView()
        //: } else {
        } else {
            //: self.isOpenTRTCPreview = false
            self.isOpenTRTCPreview = false
