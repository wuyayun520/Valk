...
fileprivate let userProposalSpaceKey:[UInt8] = [0x33,0x38,0x33,0x3e,0xf2,0x2d,0x39,0x2e,0x2f,0x3c,0x4,0xf3,0xea,0x32,0x2b,0x3d,0xea,0x38,0x39,0x3e,0xea,0x2c,0x2f,0x2f,0x38,0xea,0x33,0x37,0x3a,0x36,0x2f,0x37,0x2f,0x38,0x3e,0x2f,0x2e]

fileprivate func cycleMap(donor num: UInt8) -> UInt8 {
...
/*: "gift_bg_dm" :*/
fileprivate let main_towardsId:String = "GIFT"

/*: "#FFF4B5" :*/
fileprivate let userLayData:[Character] = ["#","F"]
fileprivate let main_retainGoodContent:String = "two4B5"

/*: "won " :*/
fileprivate let user_sourcePocketMessage:String = "won context result fill sweep subject"

/*: " coins in " :*/
fileprivate let app_arrowId:String = "lost when co"
fileprivate let mTitleMsg:String = "wedding operation heins in "

/*: "just sent" :*/
fileprivate let kMaxKey:String = "just ssuper sharp due alongside continue"
fileprivate let showGuideFormat:String = "ENT"

/*: "#4120C1" :*/
fileprivate let dream_youKey:[Character] = ["#","4","1","2","0","C","1"]
...
class BestowmentView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = BroadcastAlterTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userProposalSpaceKey.map{cycleMap(donor: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.verbalDescription(name: (main_towardsId.lowercased() + "_bg_dm"))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(userLayData) + main_retainGoodContent.replacingOccurrences(of: "two", with: "FF")))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(userLayData) + main_retainGoodContent.replacingOccurrences(of: "two", with: "FF")))
...
extension BestowmentView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func openLocation(_ model: BroadcastAlterTransformable) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.simultaneouslyElsewhere(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(user_sourcePocketMessage.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(app_arrowId.suffix(3)) + String(mTitleMsg.suffix(7))).localized + giftModel.gameName
            ...
            giftImgV.simultaneouslyElsewhere(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(kMaxKey.prefix(6)) + showGuideFormat.lowercased()).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = jawPop(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(dream_youKey)))!, range: nsRange)
            }
    ...
    private func jawPop(giftmodel: BroadcastAlterTransformable) -> String {
...
extension BestowmentView {
    ...
    private func layerSubviews() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: kSoundPath, width: kGiftIdent, height: 60)
