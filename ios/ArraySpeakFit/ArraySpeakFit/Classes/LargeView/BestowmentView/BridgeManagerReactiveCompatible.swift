...
/*: "position.x" :*/
fileprivate let mSweetText:String = "poclick"

/*: "slideIn" :*/
fileprivate let user_minimalId:[Character] = ["s","l","i","d","e","I","n"]

/*: "slideOut" :*/
fileprivate let user_investId:[Character] = ["s","l","i","d","e","O"]
fileprivate let show_arrowKey:String = "colon"
...
// __CLOSE_PRINT__
//
//  BridgeManagerReactiveCompatible.swift
//  ArraySpeakFit
//
...
class BridgeManagerReactiveCompatible: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = BridgeManagerReactiveCompatible()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [BroadcastAlterTransformable]()
    ...
    private lazy var giftView: BestowmentView = {
        //: let v = GiftFloatView()
        let v = BestowmentView()
        //: ImmediatelyThen.getWindow().addSubview(v)
        ImmediatelyThen.wow().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension BridgeManagerReactiveCompatible {
    /// 添加礼物
    //: private func addGift(gift: SocialBroadcastModel) {
    private func textUp(gift: BroadcastAlterTransformable) {
        //: self.synchronized(self) {
        self.criteriaSpring(self) {
            //: self.giftArr.append(gift)
            self.giftArr.append(gift)
            //: self.showNextGift(index: 0)
            self.betweenTotalerrupt(index: 0)
        }
    }

    //: public func synchronized<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    public func criteriaSpring<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func memberSequence(_ model: BroadcastAlterTransformable) {
        ...
        let isRTL = SolidUntilLanguageManager.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : kGiftIdent + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = kGiftIdent / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? kGiftIdent + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (mSweetText.replacingOccurrences(of: "click", with: "si") + "tion.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(user_minimalId)))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (mSweetText.replacingOccurrences(of: "click", with: "si") + "tion.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(user_investId) + show_arrowKey.replacingOccurrences(of: "colon", with: "ut")))
...
extension BridgeManagerReactiveCompatible {
    ...
    func freshNotifications() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(self.recievedNewGift(noti:)),
                                               selector: #selector(self.listenNoti(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.broadcastBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.draftAcrossForeground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.waveAngle),
    ...
    @objc private func listenNoti(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<BroadcastAlterTransformable>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != FromTransformable.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        ...
        self.textUp(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func broadcastBackground() {
    ...
    @objc private func draftAcrossForeground() {
        //: self.activeWhenShow()
        self.same()
    }
