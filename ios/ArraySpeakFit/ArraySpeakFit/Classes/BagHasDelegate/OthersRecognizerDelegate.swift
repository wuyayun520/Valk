...
fileprivate let dream_noContent:[UInt8] = [0x5,0x2,0x5,0x18,0x44,0xf,0x3,0x8,0x9,0x1e,0x56,0x45,0x4c,0x4,0xd,0x1f,0x4c,0x2,0x3,0x18,0x4c,0xe,0x9,0x9,0x2,0x4c,0x5,0x1,0x1c,0x0,0x9,0x1,0x9,0x2,0x18,0x9,0x8]

private func stackFactor(ease num: UInt8) -> UInt8 {
    return num ^ 108
}
...
// __CLOSE_PRINT__
//
//  OthersRecognizerDelegate.swift
//  AbroadTalking
//
...
class OthersRecognizerDelegate: FictionalCharacterRecognizerDelegate {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: StopPresentTransition!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = StopPresentTransition(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dream_noContent.map{stackFactor(ease: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class StopPresentTransition: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kGiftIdent, height: user_allMessage * 0.6)
    ...
    func revenueRender(corners: UIRectCorner, radius: CGFloat) {
...
extension StopPresentTransition: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = CommunicateRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension StopPresentTransition: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            incomeLess(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            ageView(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func incomeLess(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func ageView(transitionContext: UIViewControllerContextTransitioning) {
...
class CommunicateRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: StopPresentTransition!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: StopPresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kGiftIdent, height: user_allMessage)
