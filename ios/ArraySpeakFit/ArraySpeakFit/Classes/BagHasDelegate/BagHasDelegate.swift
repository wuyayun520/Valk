...
class BagHasDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: FamilyAdditiveArithmetic?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.totalItem()
            }
        }
    ...
    func hairlessness() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.semidarkness(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.colouringDramatically()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.telecom(), NSAttributedString.Key.font: UIFont.concern(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.semidarkness(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.telecom(), NSAttributedString.Key.font: UIFont.concern(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.telecom()
...
extension BagHasDelegate: UINavigationControllerDelegate {
...
extension BagHasDelegate: UIGestureRecognizerDelegate {
