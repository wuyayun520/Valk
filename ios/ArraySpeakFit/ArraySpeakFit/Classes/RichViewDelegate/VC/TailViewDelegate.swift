...
/*: "Search" :*/
fileprivate let m_earlyFaultStr:String = "mask maxSearch"

/*: "#F5F5F5" :*/
fileprivate let app_resignPath:[Character] = ["#","F","5","F","5","F","5"]

/*: "Search by UID" :*/
fileprivate let show_refMessage:String = "Searcstrike spectrum out calendar reminder"
fileprivate let app_buttonProductMsg:[Character] = ["h"]
fileprivate let userDocumentTitle:[Character] = [" ","b","y"," ","U","I","D"]

/*: "icon_search_sea" :*/
fileprivate let app_hideStyleValue:String = "icon_seremove help yesterday tip"
fileprivate let app_narrowKey:String = "team"
fileprivate let userEyeName:[Character] = ["r","c","h","_","s","e","a"]

/*: "Can’t find~" :*/
fileprivate let userBorderId:String = "fatal farCan’t "
fileprivate let dream_mineThemContent:[Character] = ["f","i","n","d","~"]

/*: "icon_kong_kong_default" :*/
fileprivate let m_anotherContent:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e"]
fileprivate let userReplaceTitle:[Character] = ["f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let mLevelStr:String = "firm basic anti makePlea"
fileprivate let appAboutUrl:String = "suspend softly enter destinationnter "
fileprivate let main_attachVeryTitle:String = "query UIDtolerance sh port"
...
class TailViewDelegate: FictionalCharacterRecognizerDelegate {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(m_earlyFaultStr.suffix(6))).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 15)
        btn.titleLabel?.font = UIFont.duringWindowExtreme(fontSize: 15)
        ...
        btn.setBackgroundImage(UIImage.gravitySize(colors: UIColor.decelerate(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(exceptElement), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(show_refMessage.prefix(5)) + String(app_buttonProductMsg) + String(userDocumentTitle)).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.verbalDescription(name: (String(app_hideStyleValue.prefix(7)) + app_narrowKey.replacingOccurrences(of: "team", with: "a") + String(userEyeName)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(EditorItemCell.self, forCellReuseIdentifier: EditorItemCell.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(userBorderId.suffix(6)) + String(dream_mineThemContent)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(m_anotherContent) + String(userReplaceTitle))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + show_diskTitle, width: kGiftIdent, height: user_allMessage - (14 + show_diskTitle + kSoundPath)), style: style)
    ...
    private lazy var reqManager: OutspokenThen = //: return OutspokenThen()
        .init()
    //: }()
}
...
extension TailViewDelegate {
    // 根据uid查询用户
    //: private func req_searchUid() {
    private func asAmong() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.changeManage(showMsg: (String(mLevelStr.suffix(4)) + "se e" + String(appAboutUrl.suffix(5)) + "the " + String(main_attachVeryTitle.prefix(9))).localized)
            //: return
            return
        }
        ...
        reqManager.endProduct(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension TailViewDelegate {
    ...
    @objc private func exceptElement() {
        //: req_searchUid()
        asAmong()
    }
...
extension TailViewDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: EditorItemCell.className(), for: indexPath) as! EditorItemCell
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.carryDoing(model: model)
...
extension TailViewDelegate: UITextFieldDelegate {
...
extension TailViewDelegate: JXPagingViewListViewDelegate {
...
extension TailViewDelegate {
    ...
    private func pointOutConstraint() {
    ...
    private func numbereractionPersonReason() {
        // 监听搜索框文本长度
