...
fileprivate let showThenOperationMsg:[UInt8] = [0x45,0x42,0x45,0x58,0x4,0x4f,0x43,0x48,0x49,0x5e,0x16,0x5,0xc,0x44,0x4d,0x5f,0xc,0x42,0x43,0x58,0xc,0x4e,0x49,0x49,0x42,0xc,0x45,0x41,0x5c,0x40,0x49,0x41,0x49,0x42,0x58,0x49,0x48]

/*: "icon_lounge" :*/
fileprivate let notiAbleIdent:[Character] = ["i","c","o","n","_"]
fileprivate let mAccelerateKey:[Character] = ["l","o","u","n","g","e"]

/*: "#777777" :*/
fileprivate let notiCaptureText:String = "#failurefailurefailure"

/*: "UID: :*/
fileprivate let dream_minimizeRemindMessage:String = "while top strikeUID:"
...
class EditorItemCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        bridgeConfinementConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showThenOperationMsg.map{$0^44}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(notiAbleIdent) + String(mAccelerateKey)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (notiCaptureText.replacingOccurrences(of: "failure", with: "77")))
        //: return lab
        return lab
        //: }()
    }()
...
extension EditorItemCell {
    /// 刷新
    //: func refreshCell(model: TalkingRecommendUserModel) {
    func carryDoing(model: UntilButterflyUserModel) {
        //: headPicImgV.setUrlImage(urlStr: model.headPic)
        headPicImgV.simultaneouslyElsewhere(urlStr: model.headPic)
        //: nicknameLab.textColor = model.loungePlus ? .userVipColor():.appTitleColor()
        nicknameLab.textColor = model.loungePlus ? .atSituationRaw() : .telecom()
        //: nicknameLab.text = model.nickname
        nicknameLab.text = model.nickname
        //: uidLab.text = "UID:\(model.uid)"
        uidLab.text = (String(dream_minimizeRemindMessage.suffix(4))) + "\(model.uid)"
...
extension EditorItemCell {
    ...
    private func bridgeConfinementConstraint() {
