...
/*: "recharge" :*/
fileprivate let kChemistryTitle:String = "rmentionc"
fileprivate let mNowMsg:String = "hadenyge"

/*: "recharge_ :*/
fileprivate let noti_serviceData:[Character] = ["r","e","c","h","a","r","g","e"]
fileprivate let showIndigenousMessage:String = "provider"

/*: ".png" :*/
fileprivate let main_listenerText:String = ".pngstage convert towards lovely translation"

/*: .png" :*/
fileprivate let user_factorIdent:String = "pitch route section.png"

/*: ".jpg" :*/
fileprivate let dreamSupportUrl:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let noti_capabilityYaOnicialData:String = ".jpgpossibility screen net react database"
...
enum EditImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class GoddamnThen: UIButton {
    ...
    var imageAlignment: EditImageAlignment = .left
...
extension UIButton {
    ...
    func labelOf(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        notTot(interval: interval, topOffset: 0)
    }
    ...
    func notTot(interval: CGFloat, topOffset: CGFloat) {
    ...
    func albescent(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func inviteLicense(color: UIColor, forState: UIControl.State) {
    ...
    func attentionStateSoil(urlStr: String,
                            //: placeImg: UIImage? = UIImage.placeImgSquare(),
                            placeImg: UIImage? = UIImage.damage(),
                            ...
                            loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: mainLastName.doGender(), with: k_filterName)
    ...
    func setFromImaginationImage(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((kChemistryTitle.replacingOccurrences(of: "mention", with: "e") + mNowMsg.replacingOccurrences(of: "deny", with: "r"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (kChemistryTitle.replacingOccurrences(of: "mention", with: "e") + mNowMsg.replacingOccurrences(of: "deny", with: "r")), with: (String(noti_serviceData) + showIndigenousMessage.replacingOccurrences(of: "provider", with: "_")) + "\(kLimitTitle)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.attentionStateSoil(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(main_listenerText.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(main_listenerText.prefix(4))), with: "_\(kLimitTitle)" + (String(user_factorIdent.suffix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(dreamSupportUrl))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(dreamSupportUrl)), with: "_\(kLimitTitle)" + (String(noti_capabilityYaOnicialData.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.attentionStateSoil(urlStr: newUrl, placeImg: nil)
    }
