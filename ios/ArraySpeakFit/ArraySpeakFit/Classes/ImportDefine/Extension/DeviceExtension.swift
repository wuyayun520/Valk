...
/*: "RSA加密失败，str:  :*/
fileprivate let noti_abstractIdent:String = "RSA\u{52a0}密失败，"
fileprivate let mSmartKey:String = "written exactly increasestr: "

/*: "weixin://" :*/
fileprivate let mainForwardPath:String = "heavy"
fileprivate let noti_managerIdent:String = "eiprofile"

/*: "dingtalk://" :*/
fileprivate let showCoordinatorEnterValue:String = "dinyesterday"

/*: "lark://" :*/
fileprivate let mainSeparateValue:[Character] = ["l","a","r","k",":"]
fileprivate let appSpecialStr:String = "//"

/*: "wxwork://" :*/
fileprivate let k_localName:String = "wxwohistory"

/*: "deviceId" :*/
fileprivate let showRemindRefreshId:[UInt8] = [0x1b,0x1a,0x9,0x16,0x1c,0x1a,0x36,0x1b]

private func calculateScene(calendar num: UInt8) -> UInt8 {
    return num ^ 127
}

/*: "weixin" :*/
fileprivate let mServiceValue:String = "weobstaclexobstaclen"

/*: "wxwork" :*/
fileprivate let app_toExitMessage:String = "heatwork"

/*: "dingtalk" :*/
fileprivate let m_birthdayPanelName:[Character] = ["d","i","n","g","t","a","l"]
fileprivate let showCoalMessage:[Character] = ["k"]

/*: "lark" :*/
fileprivate let m_appealPath:[Character] = ["l","a","r","k"]
...
extension UIDevice {
    ...
    class func important(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.quantitativeRelationCan())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func quantitativeRelationCan() -> String {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func iconThemeStr(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (noti_abstractIdent + String(mSmartKey.suffix(5))) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (mainForwardPath.replacingOccurrences(of: "heavy", with: "w") + noti_managerIdent.replacingOccurrences(of: "profile", with: "xi") + "n://")
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (showCoordinatorEnterValue.replacingOccurrences(of: "yesterday", with: "g") + "talk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(mainSeparateValue) + appSpecialStr.capitalized)
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (k_localName.replacingOccurrences(of: "history", with: "r") + "k://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(notiResolutionMatchMessage) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: notiResolutionMatchMessage)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = SweepAddrTool.switcheOne() || SweepAddrTool.towardAppVpn()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = iconThemeStr(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: showRemindRefreshId.map{calculateScene(calendar: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((mServiceValue.replacingOccurrences(of: "obstacle", with: "i")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((app_toExitMessage.replacingOccurrences(of: "heat", with: "wx")))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(m_birthdayPanelName) + String(showCoalMessage)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((String(m_appealPath)))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
