...
@implementation UIImage (CampaignCompress)

//: - (NSData *)headMargin {
- (NSData *)headMargin {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.penetrateDistance;
...
- (UIImage *)custom {
    ...
    //: return [UIImage imageWithData:[self headMargin]];
    return [UIImage imageWithData:[self headMargin]];
}
...
- (UIImage *)judge {
    ...
    return [self object:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)penetrateDistance {
    //: return [self wcCompres:NO];
    return [self object:NO];
}
...
- (UIImage *)object:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self sturdiness:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self isoclinic:size];
...
- (CGSize)sturdiness:(Boolean)isSession {
...
+ (NSData *)intervalernalRepresentation:(UIImage *)image sample:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image length:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG handleReport:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG scale:scalesize resizeTo:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG handleReport:(fileSize*1024)];
    }
...
- (UIImage*)length:(CGSize)targetSize{
...
- (UIImage *)scale:(CGSize)targetSize resizeTo:(BOOL)isScale{
...
- (NSData *)handleReport:(NSInteger)fileSize{
...
- (UIImage *)isoclinic:(CGSize)newSize {
