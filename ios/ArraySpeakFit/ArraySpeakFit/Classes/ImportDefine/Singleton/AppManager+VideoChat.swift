...
/*: "type" :*/
fileprivate let mainMemoryKey:[Character] = ["t","y","p","e"]

/*: "You are currently on a call, please try again later" :*/
fileprivate let k_dueIdent:[UInt8] = [0x7d,0x4b,0x51,0x4,0x45,0x56,0x41,0x4,0x47,0x51,0x56,0x56,0x41,0x4a,0x50,0x48,0x5d,0x4,0x4b,0x4a,0x4,0x45,0x4,0x47,0x45,0x48,0x48,0x8,0x4,0x54,0x48,0x41,0x45,0x57,0x41,0x4,0x50,0x56,0x5d,0x4,0x45,0x43,0x45,0x4d,0x4a,0x4,0x48,0x45,0x50,0x41,0x56]

/*: "onCallResume" :*/
fileprivate let dreamAnswerFoundationText:String = "add resent exceptonCall"
fileprivate let userFiniteTitle:String = "loose"
fileprivate let show_ownerTitle:String = "esuoutside"
...
extension AppManager {
    ...
    func outerSpace(info: [String: Any], situation: RelatedChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(String(mainMemoryKey))].intValue
...
extension AppManager {
    ...
    private func belowBrig(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isObject {
        if !resumeTalk && StackSocketManager.shared.isObject {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.changeManage(showMsg: String(bytes: k_dueIdent.map{$0^36}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if decideBroad()!.isKind(of: FourthViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            decideBroad()!.isKind(of: StackErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            decideBroad()!.isKind(of: GuideViewController.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! FourthViewController).chatModel = OthersChatModel.scienceLab(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = StackErrorDelegate()
            ...
            (vc as! StackErrorDelegate).chatModel = OthersChatModel.scienceLab(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(dreamAnswerFoundationText.suffix(6)) + userFiniteTitle.replacingOccurrences(of: "loose", with: "R") + show_ownerTitle.replacingOccurrences(of: "outside", with: "me"))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.priorityCurrency(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func priorityCurrency(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = decideBroad() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = ImmediatelyThen.getWindow().rootViewController as! UITabBarController
            let rootVc = ImmediatelyThen.wow().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(StackErrorDelegate.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(FourthViewController.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: FourthViewController?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! FourthViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? FourthViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.statement(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: BagHasDelegate.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! BagHasDelegate
            ...
            if topVc != nil, topVc!.isKind(of: AgencyControllerReactiveCompatible.self) {
            ...
            var currentChatVc: StackErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! StackErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? StackErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.collectionDown(resumeData: resumeData)
                }
                //: return
                return
            }
