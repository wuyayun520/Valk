...
/*: "http://stat. :*/
fileprivate let notiPerspectiveKey:String = "HTTP"
fileprivate let user_hungKey:[Character] = ["."]

/*: .com" :*/
fileprivate let kTenderMessage:String = "result invitation variety compete.com"

/*: "http://" :*/
fileprivate let dream_grayChooseUrl:String = "salt broadcast required begin takehttp://"

/*: "https://" :*/
fileprivate let mainCombineTaName:[Character] = ["h","t","t"]
fileprivate let noti_thenName:String = "least allow may listenerps://"

/*: "http://m. :*/
fileprivate let app_sameMsg:String = "hobserverobserverp"
fileprivate let appThingData:[Character] = [":","/","/","m","."]

/*: "http:// :*/
fileprivate let mainTempManualTitle:String = "http://nag origin doc recommendation agree"

/*: "3,90" :*/
fileprivate let userResourceName:[Character] = ["3",",","9","0"]

/*: "1990-01-01" :*/
fileprivate let noti_prosecutorIdent:String = "1990-"

/*: "MM-dd-yyyy" :*/
fileprivate let kCombinedThatFormat:[Character] = ["M"]
fileprivate let showListMsg:[Character] = ["M","-","d","d","-","y","y","y","y"]
...
class ExecLaborReactiveCompatible: NSObject {
    ...
    var requestType: OthersRequestType?
    ...
    override init() {
        ...
        self.requestServer = SweepAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func smoothOutObject() -> Bool {
...
struct BubbleSolidMeasurable: HandyJSON {
    //: var errno: Int!
...
public struct PeopleErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct DumpFromConfigModel: HandyJSON {
    ...
    var CSConfig = TailAgencyConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (notiPerspectiveKey.lowercased() + "://stat" + String(user_hungKey)) + "\(k_filterName)" + (String(kTenderMessage.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(dream_grayChooseUrl.suffix(7)))) && !statDomain.hasPrefix((String(mainCombineTaName) + String(noti_thenName.suffix(5)))) {
                //: return "http://".appending(statDomain)
                return (String(dream_grayChooseUrl.suffix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (app_sameMsg.replacingOccurrences(of: "observer", with: "t") + String(appThingData)) + "\(k_filterName)" + (String(kTenderMessage.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(mainTempManualTitle.prefix(7))) + "\(urlDomain)"
        }
    }
...
public class CharacterTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [BubbleTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [BubbleTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [BubbleTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [RightAwayModelType]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [SocietalHandyJSON]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = LimitSmallHandyJSON() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class NoteHandyJSON: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [PoTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [PoTagModel]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [AudienceMeasurable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (String(userResourceName)) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.struggle(self.birthday ?? (noti_prosecutorIdent.capitalized + "01-01"), dateFormat: (String(kCombinedThatFormat) + String(showListMsg)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.dayComponents().year else { return "0" }
...
struct AudienceMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct PoTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func popularBoyfriend(compareTo: PoTagModel) -> Bool {
        //: return
        return
...
struct EditorMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [EliteTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [EliteTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct EliteTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
...
struct TailAgencyConfigModel: HandyJSON {
...
struct RightAwayModelType: HandyJSON {
...
struct LimitSmallHandyJSON: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
