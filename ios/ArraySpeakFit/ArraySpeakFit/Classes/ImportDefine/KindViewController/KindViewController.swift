...
protocol PeopleTailNavigationDelegate: AnyObject {
    /// current selected information.
    /// - Parameters:
    ///   - viewController: selected VC.
    ///   - index: current
    //: func selectedViewController(_ viewController: UIViewController, index: Int)
    func succeed(_ viewController: UIViewController, index: Int)
}
...
class KindViewController: UIViewController {
    ...
    weak var delegate: PeopleTailNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = ImmediatelyThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = ImmediatelyThen.dataHeight() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= dreamOffValue
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: ImmediatelyThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: ImmediatelyThen.poundWidthGet(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(ImmediatelyThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(ImmediatelyThen.poundWidthGet()), height: 0)
        ...
        scrollView.semanticContentAttribute = SolidUntilLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: ImmediatelyThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: ImmediatelyThen.poundWidthGet()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: ImmediatelyThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: ImmediatelyThen.poundWidthGet()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = SolidUntilLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(curst), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = ImmediatelyThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = ImmediatelyThen.poundWidthGet() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(ImmediatelyThen.getScreenWidth()-originX-titleWidth)
            let x = SolidUntilLanguageManager.shared.direction == .leftToRight ? originX:(ImmediatelyThen.poundWidthGet()-originX-titleWidth)
...
extension KindViewController {
    ...
    @objc private func curst(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        searched(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * ImmediatelyThen.getScreenWidth()
        let offset_x = CGFloat(index) * ImmediatelyThen.poundWidthGet()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        bend(withIndex: index)
        //: seletedViewController(withIndex: index)
        kind(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func bend(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * ImmediatelyThen.getScreenWidth()
        let origin_x = CGFloat(index) * ImmediatelyThen.poundWidthGet()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: ImmediatelyThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: ImmediatelyThen.poundWidthGet(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func searched(_ button: UIButton) {
    ...
    func afterRefrigerator(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= ImmediatelyThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= ImmediatelyThen.poundWidthGet() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - ImmediatelyThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - ImmediatelyThen.poundWidthGet() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - ImmediatelyThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - ImmediatelyThen.poundWidthGet()
...
extension KindViewController {
    ...
    func kind(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.succeed(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension KindViewController: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / ImmediatelyThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / ImmediatelyThen.poundWidthGet())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            searched(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        bend(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / ImmediatelyThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / ImmediatelyThen.poundWidthGet())
        ...
        //: let scaleR = scrollView.contentOffset.x / ImmediatelyThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / ImmediatelyThen.poundWidthGet() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
