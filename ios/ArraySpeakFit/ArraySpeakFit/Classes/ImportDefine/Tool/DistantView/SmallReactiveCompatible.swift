...
fileprivate let dream_roundingMsg:[UInt8] = [0x7,0x0,0x7,0x1a,0x46,0xd,0x1,0xa,0xb,0x1c,0x54,0x47,0x4e,0x6,0xf,0x1d,0x4e,0x0,0x1,0x1a,0x4e,0xc,0xb,0xb,0x0,0x4e,0x7,0x3,0x1e,0x2,0xb,0x3,0xb,0x0,0x1a,0xb,0xa]

private func enthusiasmSoftly(now num: UInt8) -> UInt8 {
    return num ^ 110
}

/*: "FFD855" :*/
fileprivate let appSecondUrl:String = "coordinatorD855"
...
typealias CropSlidingHandler = (_ direction: CoveredBridgeSlideDirection) -> Void
//: typealias CropSlidingEndHandler = () -> Void
typealias CropSlidingEndHandler = () -> Void

//: enum TalkingSlideDirection {
enum CoveredBridgeSlideDirection {
    //: case left
    case left
    //: case right
    case right
...
class SmallReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - appListId
                ...
                self.reloadStatement(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_roundingMsg.map{enthusiasmSoftly(now: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (appSecondUrl.replacingOccurrences(of: "coordinator", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: mainEndContent, y: 0, width: appListId, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - appListId, y: 0, width: appListId, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.optimize(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: mainEndContent, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.optimize(0, 0, 0, 0.6)
...
extension SmallReactiveCompatible {
    ...
    func reloadStatement(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - appListId,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - appListId)
    }
}
...
extension SmallReactiveCompatible {
    ...
    @objc func completeDown(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.reloadStatement(startX: videoStartX)
    ...
    @objc func maleOn(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension SmallReactiveCompatible {
    ...
    private func baseballTeam() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(completeDown(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(maleOn(gesture:)))
    ...
    func systemOfRulesXposition(_ xposition: CGFloat) {
