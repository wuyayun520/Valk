...
fileprivate let showEnjoyMessage:[UInt8] = [0xba,0xbd,0xba,0xa7,0xfb,0xb0,0xbc,0xb7,0xb6,0xa1,0xe9,0xfa,0xf3,0xbb,0xb2,0xa0,0xf3,0xbd,0xbc,0xa7,0xf3,0xb1,0xb6,0xb6,0xbd,0xf3,0xba,0xbe,0xa3,0xbf,0xb6,0xbe,0xb6,0xbd,0xa7,0xb6,0xb7]

private func checkSumo(instruction num: UInt8) -> UInt8 {
    return num ^ 211
}
...
typealias SlidingHandler = (_ direction: CoveredBridgeSlideDirection) -> Void
...
class DistantView: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (mainEndContent - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showEnjoyMessage.map{checkSumo(instruction: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.componentRe(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: SmallReactiveCompatible = {
        ...
        let overlay = SmallReactiveCompatible(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension DistantView {
    ...
    private func componentRe(point: CGFloat) -> UIImage {
...
extension DistantView: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = mainEndContent - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.stageExit(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.stageExit(isShow: true)
    }
...
extension DistantView {
    ...
    private func subviewsOpen() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(mainEndContent)
