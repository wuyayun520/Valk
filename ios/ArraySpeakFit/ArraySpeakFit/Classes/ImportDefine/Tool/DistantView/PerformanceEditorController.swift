...
fileprivate let showCleanEyeUrl:[UInt8] = [0x4b,0x50,0x4b,0x56,0xa,0x45,0x51,0x46,0x47,0x54,0x1c,0xb,0x2,0x4a,0x43,0x55,0x2,0x50,0x51,0x56,0x2,0x44,0x47,0x47,0x50,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x50,0x56,0x47,0x46]

fileprivate func libraryFirm(permission num: UInt8) -> UInt8 {
...
fileprivate let m_roofName:String = "btnJQstretch imagination recommendation"
fileprivate let kPatientMsg:String = "not compute scale stickihw_k"
fileprivate let mSirKey:String = "ADOW"

/*: "Edit" :*/
fileprivate let appPartnerTravelKey:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let noti_directMessage:String = "Finishpure hurry"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let dream_faultMineIdent:String = "that manualbtnYDo"
fileprivate let dream_productKey:String = "income purpose explorationoediv_"
fileprivate let mainDisplayData:String = "pressay"

/*: "Cancel" :*/
fileprivate let dream_findBrandMutualValue:String = "Cancelequity number stop boyfriend three"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let user_pathId:String = "remote currency media heatbgQkR"
fileprivate let kProcessingFormat:String = "among favorite arrivaloediv_t"
fileprivate let dream_kitPath:[Character] = ["o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let mTotalFormat:[UInt8] = [0xb5,0x89,0x84,0xc1,0x97,0x88,0x85,0x84,0x8e,0xc1,0x8d,0x84,0x8f,0x86,0x95,0x89,0xc1,0x92,0x89,0x8e,0x94,0x8d,0x85,0xc1,0x83,0x84,0xc1,0xc4,0xa1,0xcc,0xc4,0xa1,0xc1,0x92,0x84,0x82,0x8e,0x8f,0x85,0x92,0xcf,0xc1,0xb1,0x8d,0x84,0x80,0x92,0x84,0xc1,0x88,0x8f,0x95,0x84,0x93,0x82,0x84,0x91,0x95,0xc1,0x88,0x95]

private func topPiece(fence num: UInt8) -> UInt8 {
    return num ^ 225
}
...
// __CLOSE_PRINT__
//
//  PerformanceEditorController.swift
//  AbroadTalking
//
...
typealias VideoEditBlock = (_ videoInfo: GeneticInfoModel?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let m_replaceServerStr: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class PerformanceEditorController: FictionalCharacterRecognizerDelegate {
    ...
    private var slider: DistantView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = GeneticInfoModel() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = GraceEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isObject == true {
        if StackSocketManager.shared.isObject == true { // 正在音视频通话，静音播放
    ...
    init(style: GraceEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kGiftIdent - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showCleanEyeUrl.map{libraryFirm(permission: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.verbalDescription(name: (String(m_roofName.prefix(5)) + "bEAB_et" + String(kPatientMsg.suffix(5)) + "cab_sh" + mSirKey.lowercased())), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = SolidUntilLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(torso), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(appPartnerTravelKey)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.storageEngine(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.duringWindowExtreme(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = SolidUntilLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(noti_directMessage.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.equitySize(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.storageEngine()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(socialArrival), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.verbalDescription(name: (String(dream_faultMineIdent.suffix(6)) + "2AS_" + String(dream_productKey.suffix(6)) + mainDisplayData.replacingOccurrences(of: "press", with: "pl"))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(heaviness(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(dream_findBrandMutualValue.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(scanStroke), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.verbalDescription(name: (String(user_pathId.suffix(5)) + "TbF_" + String(kProcessingFormat.suffix(7)) + String(dream_kitPath))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: mTotalFormat.map{topPiece(fence: $0)}, encoding: .utf8)!.launchRid(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: ThingThen = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = ThingThen(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension PerformanceEditorController {
    /// 返回按钮事件
    ...
    private func sirPop(animated _: Bool = true) {
    ...
    @objc func bassAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kGiftIdent - 2 * 26, height: user_allMessage - 46 - 127 - user_sizeValue)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kGiftIdent / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: user_allMessage, width: kGiftIdent, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = DistantView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: m_replaceServerStr)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - user_sizeValue)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: CoveredBridgeSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func socialArrival() {
        ...
        if self.videoEditHandler != nil {
            ...
            SweepBridgeThen.shared.whoPerson(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.sirPop()
    ...
    @objc func heaviness(sender: UIButton) {
...
extension PerformanceEditorController {
    ...
    private func queryParticipant() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(keyCapture), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.systemOfRulesXposition(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func proposal() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + user_sizeValue)
        }
    ...
    private func soundNotification() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(goQuantityo),
