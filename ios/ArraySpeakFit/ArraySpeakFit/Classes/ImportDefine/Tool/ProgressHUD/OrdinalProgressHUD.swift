...
fileprivate let showEqualId:[UInt8] = [0x92,0x95,0x92,0x8f,0xd3,0x98,0x94,0x9f,0x9e,0x89,0xc1,0xd2,0xdb,0x93,0x9a,0x88,0xdb,0x95,0x94,0x8f,0xdb,0x99,0x9e,0x9e,0x95,0xdb,0x92,0x96,0x8b,0x97,0x9e,0x96,0x9e,0x95,0x8f,0x9e,0x9f]

private func boyfriendEnvironment(less num: UInt8) -> UInt8 {
    return num ^ 251
}
...
let dreamImageData = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let k_busyIdent = 14.0
//: let kProgressHUD_alpha        = 0.9
let app_failData = 0.9
//: let kBackgroundView_alpha     = 0.6
let userControlCoverName = 0.6
//: let kAnimationInterval        = 0.2
let main_keyIdent = 0.2
//: let kTransformScale           = 0.9
let kInfoData = 0.9

//: open class ProgressHUD: UIView {
open class OrdinalProgressHUD: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showEqualId.map{boyfriendEnvironment(less: $0)}, encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = OrdinalProgressHUD()
    ...
    class func loo() {
        //: show(superView: nil)
        upInclude(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func upInclude(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                OrdinalProgressHUD.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                OrdinalProgressHUD.shared.activityIndicator.center = OrdinalProgressHUD.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(OrdinalProgressHUD.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                OrdinalProgressHUD.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                OrdinalProgressHUD.shared.activityIndicator.center = OrdinalProgressHUD.shared.center
                //: ImmediatelyThen.getWindow().addSubview(ProgressHUD.shared)
                ImmediatelyThen.wow().addSubview(OrdinalProgressHUD.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        OrdinalProgressHUD.shared.notAd()
    }

    //: class func dismiss() {
    class func vision() {
        //: ProgressHUD.shared.hud_stopAnimating()
        OrdinalProgressHUD.shared.restrictionFragment()
    }
    ...
    private func restrictionFragment() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: main_keyIdent) {
                ...
                OrdinalProgressHUD.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: dreamImageData, height: dreamImageData)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = k_busyIdent
...
extension OrdinalProgressHUD {
    //: class func toast(_ str: String?) {
    class func mDevice(_ str: String?) {
    ...
    class func unlessTime(_ str: String?, showTime: CGFloat) {
        ...
        titleLab.textColor = .white
        //: ImmediatelyThen.getWindow().addSubview(titleLab)
        ImmediatelyThen.wow().addSubview(titleLab)
        //: let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        //: titleLab.center = ImmediatelyThen.getWindow().center
        titleLab.center = ImmediatelyThen.wow().center
