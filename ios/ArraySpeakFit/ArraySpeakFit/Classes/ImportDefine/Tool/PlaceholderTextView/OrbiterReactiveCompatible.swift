...
// __CLOSE_PRINT__
//
//  OrbiterReactiveCompatible.swift
//  OrbiterReactiveCompatible
...
struct SourceTransportLabelOrigin {
    //: let x =  5.0
    let x = 5.0
    //: let y =  5.0
    let y = 5.0
...
struct PreviouslyContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class OrbiterReactiveCompatible: UIView {
    // MARK: - 懒加载属性
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        colouredOn(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        colouredOn(placeholder: nil, placeholderColor: nil)
    }
}
...
extension OrbiterReactiveCompatible {
    /// placeholder Label Setup
    //: private func setup(placeholder: String?, placeholderColor: UIColor?) {
    private func colouredOn(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(SourceTransportLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(SourceTransportLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-SourceTransportLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: PreviouslyContainerInset().top, left: PreviouslyContainerInset().left, bottom: PreviouslyContainerInset().bottom, right: PreviouslyContainerInset().right)
...
extension OrbiterReactiveCompatible: UITextViewDelegate {
