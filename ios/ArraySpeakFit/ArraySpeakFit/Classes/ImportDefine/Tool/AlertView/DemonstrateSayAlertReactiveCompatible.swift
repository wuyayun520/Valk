...
fileprivate let dream_expressSweepUrl:[Character] = ["b","t","n","_","s","h","o","w","_","a","l","e","r","t","B","a","c","k","g","r"]
fileprivate let showHeatFormat:[Character] = ["o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let app_thingStr:String = "view not bring modebtn_al"
fileprivate let dreamSeatConstraintData:String = "confirmkgr"
fileprivate let dream_bulkOriginalTitle:[Character] = ["_","f","a","l","s","e"]
...
// __CLOSE_PRINT__
//
//  DemonstrateSayAlertReactiveCompatible.swift
//  AbroadTalking
//
...
public class DemonstrateSayAlertReactiveCompatible: NSObject {
    /// 通用回调
...
public extension DemonstrateSayAlertReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: SeeAlertConfig) -> Void
    ...
    internal class func rightUsed(title: String? = nil,
                                  //: message: String?  = nil,
                                  message: String? = nil,
                                  ...
                                  config: SeeAlertConfig? = nil)
    {
        //: customAlert(title: title,
        alterConfig(title: title,
                    //: message: message,
                    message: message,
    ...
    @objc internal class func pickOut(title: String? = nil,
    ...
    class func alterConfig(title: String? = nil,
                           ...
                           config: SeeAlertConfig? = nil)
    {
        ...
        let alertView = CommunicateAlertView(title: title,
        ...
        //: ImmediatelyThen.getWindow().addSubview(alertView)
        ImmediatelyThen.wow().addSubview(alertView)
    ...
    class func paidVacation() {
        //: ImmediatelyThen.getWindow().subviews.forEach { (view) in
        ImmediatelyThen.wow().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: CommunicateAlertView.self) {
    ...
    class func sourceLanguage(title: String, isTouch: Bool) {
        //: ImmediatelyThen.getWindow().subviews.forEach { (view) in
        ImmediatelyThen.wow().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: CommunicateAlertView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: CommunicateAlertView = view as! CommunicateAlertView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.backVisual(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class SeeAlertConfig {
    ...
    public var maskType: CycleMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .concern(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.telecom()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .concern(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.telecom()
    ...
    public var buttonFont: UIFont = .concern(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.storageEngine()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(dream_expressSweepUrl) + String(showHeatFormat))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(app_thingStr.suffix(6)) + "ertBa" + dreamSeatConstraintData.replacingOccurrences(of: "confirm", with: "c") + "ound" + String(dream_bulkOriginalTitle))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .concern(type: .Medium, fontSize: 16)
