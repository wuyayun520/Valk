...
/*: "Select a date" :*/
fileprivate let noti_stillId:String = "appear gravity course goSelec"
fileprivate let show_comeMotivationKey:[Character] = ["t"," ","a"]
fileprivate let userTimingKey:String = " datenotice represent"
...
// __CLOSE_PRINT__
//
//  AlongsideGraceReactiveCompatible.swift
//  AbroadTalking
//
...
protocol RelatedViewDelegate: NSObjectProtocol {
...
class AlongsideGraceReactiveCompatible: QualityThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: RelatedViewDelegate?
    ...
    override func giftWrap() {
        ...
        titleString = (String(noti_stillId.suffix(5)) + String(show_comeMotivationKey) + String(userTimingKey.prefix(5))).localized
        ...
        pendingHinduCalendarMonth(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func pendingHinduCalendarMonth(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func railroad(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.license(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.license(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return enableRead(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func enableRead(year: Int, month: Int) -> Int {
