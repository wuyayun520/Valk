...
@interface PrimeData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation PrimeData

+ (instancetype)sharedInstance {
    static PrimeData *instance = nil;
...
+ (NSData *)PrimeDataToData:(NSString *)value {
...
- (Byte *)PrimeDataToCache:(Byte *)data {
    int monitorPackage = data[0];
    Byte surgery = data[1];
    int still = data[2];
    for (int i = still; i < still + monitorPackage; i++) {
        int value = data[i] - surgery;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    }
    data[still + monitorPackage] = 0;
    return data + still;
}

- (NSString *)StringFromPrimeData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self PrimeDataToCache:data]];
}

//: Height
- (NSString *)mPlusData {
    /* static */ NSString *mPlusData = nil;
    if (!mPlusData) {
		NSString *origin = @"062d07ec8a78627592969495a130";
		NSData *data = [PrimeData PrimeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        mPlusData = [self StringFromPrimeData:value];
    }
    return mPlusData;
}

//: 请打开相机权限
- (NSString *)user_pressureMessage {
    /* static */ NSString *user_pressureMessage = nil;
    if (!user_pressureMessage) {
		NSString *origin = @"15270719e7329c0fd6de0db0ba0ce3a70ec2df0dc3e10dc4aa10c0b7d0";
		NSData *data = [PrimeData PrimeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        user_pressureMessage = [self StringFromPrimeData:value];
    }
    return user_pressureMessage;
}

//: Width
- (NSString *)userEaseMobileValue {
    /* static */ NSString *userEaseMobileValue = nil;
    if (!userEaseMobileValue) {
		NSString *origin = @"056103b8cac5d5c9da";
		NSData *data = [PrimeData PrimeDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        userEaseMobileValue = [self StringFromPrimeData:value];
    }
    return userEaseMobileValue;
}
...
@interface DramaticallyExplore () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation DramaticallyExplore

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithCapacity:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        format:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   presentation:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         path:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self anomaly:iDevicePosition];
...
- (void)format:(CGPoint)point centerAcrossFrom:(CGRect)frame
{
    ...
    [self status:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           text:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                systemPoint:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)status:(AVCaptureFocusMode)focusMode text:(AVCaptureExposureMode)exposureMode systemPoint:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self resign:^(AVCaptureDevice *captureDevice) {
...
- (void)resign:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self anomaly:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self resolve]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[[PrimeData sharedInstance] userEaseMobileValue]] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[[PrimeData sharedInstance] mPlusData]] floatValue];
...
- (CGRect)limitColor:(CGRect)rect quantitativeRelation:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[[PrimeData sharedInstance] userEaseMobileValue]] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[[PrimeData sharedInstance] mPlusData]] floatValue];
...
- (BOOL)resolve
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[[PrimeData sharedInstance] user_pressureMessage]
...
- (AVCaptureDevice *)anomaly:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(tap:insert:estimatedConnection:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate tap:captureOutput insert:sampleBuffer estimatedConnection:connection];
        }
    }
