...
/*: "SENSEME" :*/
fileprivate let mManageBindKey:[Character] = ["S","E","N","S","E","M","E"]

/*: "lic" :*/
fileprivate let kPopTitle:String = "commandc"

/*: "mf/Index/senseTime" :*/
fileprivate let user_trackTitle:String = "condition publisher thanks asset sharedmf/Ind"
fileprivate let appDogPath:String = "except challenge vision averageenseTime"

/*: "url" :*/
fileprivate let userCapableLockOnPath:[Character] = ["u","r","l"]

/*: "md5" :*/
fileprivate let dream_alterDirectMsg:[Character] = ["m","d","5"]

/*: "SENSEMELIC" :*/
fileprivate let mBelowObstacleValue:[Character] = ["S","E","N","S","E"]
fileprivate let kThatUrl:String = "melic"

/*: "App/ST/" :*/
fileprivate let m_screenPath:[Character] = ["A","p","p","/","S","T","/"]

/*: "GET" :*/
fileprivate let userListenKey:String = "get"
...
//: @objcMembers public class MortalityThen: NSObject {
@objcMembers public class MortalityThen: NSObject {
    //: static public let share = MortalityThen()
    public static let share = MortalityThen()
    //: private override init() {}
    override private init() {}

    // 校验license有效性 先本地->后网络
    //: public func checkLicense() -> Bool {
    public func scramble() -> Bool {
    ...
    func hesitate() -> Bool {
        ...
        if let path = ButterflyPodspecBundle.bundle.path(forResource: (String(mManageBindKey)), ofType: (kPopTitle.replacingOccurrences(of: "command", with: "li"))) {
            //: let licenseData = NSData(contentsOfFile: path)
            let licenseData = NSData(contentsOfFile: path)
            //: if licenseData != nil {
            if licenseData != nil {
                //: return checkActiveCodeWithData(dataLicense: licenseData!)
                return untilTransfer(dataLicense: licenseData!)
            }
        }
    ...
    func underFollow() -> Bool {
        //: let path = licDiskPath()
        let path = saltworksPath()
        ...
        if FileManager.default.fileExists(atPath: path as String) {
            ...
            if licData != nil {
                ...
                return untilTransfer(dataLicense: licData!)
            }
        }
    ...
    func untilTransfer(dataLicense: NSData) -> Bool {
...
//: extension MortalityThen {
extension MortalityThen {
    // 获取美颜license
    //: public func checkRemoteLicInfoWith(completion: @escaping Completionblock) {
    public func mediumTo(completion: @escaping Completionblock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = ExecLaborReactiveCompatible()
        //: reqModel.requestPath = "mf/Index/senseTime"
        reqModel.requestPath = (String(user_trackTitle.suffix(6)) + "ex/s" + String(appDogPath.suffix(8)))
        ...
        user_topEnvironmentId.thumbPrompt(model: reqModel) { _, result, _ in
            ...
            let licLink = json[(String(userCapableLockOnPath))].stringValue
            //: let md5 = json["md5"].stringValue
            let md5 = json[(String(dream_alterDirectMsg))].stringValue
            //: if self.checkLicFileMd5With(md5: md5 as NSString) == false {
            if self.resolvePull(md5: md5 as NSString) == false {
                //: if  licLink.count > 0 {
                if licLink.count > 0 {
                    //: self.downloadLicsenWith(licUrl: licLink as NSString, completion: completion)
                    self.downloadInsideBarn(licUrl: licLink as NSString, completion: completion)
                    //: return
                    return
                }
            }
    ...
    func resolvePull(md5: NSString) -> Bool {
        //: if md5.length <= 0 {
        if md5.length <= 0 {
            //: return false
            return false
        ...
        let filePath = saltworksPath()
        //: let fileMd5: NSString = TalkingGiftAnimatTool.shared.fileMD5(filePath: filePath as String) as NSString
        let fileMd5: NSString = RelatedAnimatTool.shared.trademarkPath(filePath: filePath as String) as NSString
    ...
    func saltworksPath() -> NSString {
        //: let stDirectory = licDiskDirectory()
        let stDirectory = detailDisk()
        //: return stDirectory.appendingPathComponent("SENSEMELIC") as NSString
        return stDirectory.appendingPathComponent((String(mBelowObstacleValue) + kThatUrl.uppercased())) as NSString
    }

    //: func licDiskDirectory() -> NSString {
    func detailDisk() -> NSString {
        ...
        let stDirectory = docDirectory.appendingPathComponent((String(m_screenPath)))
    ...
    func downloadInsideBarn(licUrl: NSString, completion: @escaping Completionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (userListenKey.uppercased())
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                if data != nil {
                    ...
                    self.viaData(licData: data! as NSData)
                    //: licValid = self.checkDownloadedLicense()
                    licValid = self.underFollow()
                }
