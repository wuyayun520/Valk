...
/*: "filterSuite" :*/
fileprivate let dream_allFormat:String = "filterSfinish seat"
fileprivate let app_resMessage:[Character] = ["u","i","t","e"]

/*: "customSuite" :*/
fileprivate let main_generateFormat:String = "customSremove field join net select"
fileprivate let m_camData:String = "uiinfrastructuree"

/*: "App/TarImmediatelyCacheCost" :*/
fileprivate let m_mineText:[Character] = ["A","p","p","/","S","T","F","i","l","t","e","r","C","a","c"]
fileprivate let appExploreIdent:String = "extension interestheCost"
...
//: @objcMembers public class TarImmediatelyCacheCost: NSObject, NSSecureCoding {
@objcMembers public class TarImmediatelyCacheCost: NSObject, NSSecureCoding {
    //: var curBeautySuite: BeautyItemType?
    var curBeautySuite: AboardMagnitudeervalLiteral?
    //: var curFeature: STEffectsType?
    var curFeature: EffectsColumnConvertible?
    /// 用户的滤镜设置
    //: public var filterSuite: STFilterEntiretycostModel?
    public var filterSuite: OpticModel?
    /// 滤镜列表模型数组
    //: var filterModelArray = Array<STFilterLooksModel>.init()
    var filterModelArray = Array<PerformanceThen>.init()

    //: var customSuite =  STFilterEntiretycostModel.init()
    var customSuite = OpticModel() // 用户自定义的美颜

    //: static public let share = TarImmediatelyCacheCost()
    public static let share = TarImmediatelyCacheCost()
    ...
    public required init?(coder: NSCoder) {
        ...
        let filDict = coder.decodeObject(forKey: (String(dream_allFormat.prefix(7)) + String(app_resMessage)))
        //: let custDict =  coder.decodeObject(forKey: "customSuite")
        let custDict = coder.decodeObject(forKey: (String(main_generateFormat.prefix(7)) + m_camData.replacingOccurrences(of: "infrastructure", with: "t")))
        //: if filDict != nil {
        if filDict != nil {
            //: filterSuite = try! STFilterEntiretycostModel.init(dictionary: filDict as? [AnyHashable: Any])
            filterSuite = try! OpticModel(dictionary: filDict as? [AnyHashable: Any])
        }
        //: if custDict != nil {
        if custDict != nil {
            //: customSuite = try! STFilterEntiretycostModel.init(dictionary: custDict as? [AnyHashable: Any])
            customSuite = try! OpticModel(dictionary: custDict as? [AnyHashable: Any])
        }
    }
    ...
    public func encode(with coder: NSCoder) {
        ...
        coder.encode(filDict, forKey: (String(dream_allFormat.prefix(7)) + String(app_resMessage)))
        //: coder.encode(custDict, forKey: "customSuite")
        coder.encode(custDict, forKey: (String(main_generateFormat.prefix(7)) + m_camData.replacingOccurrences(of: "infrastructure", with: "t")))
    }
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(invariant(sender:)), name: dream_imagePath, object: nil)
    }
...
//: @objc extension TarImmediatelyCacheCost {
@objc extension TarImmediatelyCacheCost {
    /// 获取缓存
    //: public func getLocalModel() {
    public func makeModel() {
        //: let path = self.cachePath()
        let path = self.cacheAfter()
        //: if FileManager.default.fileExists(atPath: path) {
        if FileManager.default.fileExists(atPath: path) {
            //: var obj: TarImmediatelyCacheCost?
            var obj: TarImmediatelyCacheCost?
            ...
            if #available(iOS 11.0, *) {
                ...
                do {
                    ...
                    let data = try Data(contentsOf: URL(fileURLWithPath: path))
                    //: if let model: TarImmediatelyCacheCost =  try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? TarImmediatelyCacheCost {
                    if let model: TarImmediatelyCacheCost = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? TarImmediatelyCacheCost {
                        //: obj  = model
                        obj = model
                    }
                ...
                //: let model: TarImmediatelyCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? TarImmediatelyCacheCost
                let model: TarImmediatelyCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? TarImmediatelyCacheCost
                //: if  model != nil {
                if model != nil {
                    //: obj  = model
    ...
    public func basePress() -> OpticModel {
    ...
    func signCounto() {
        ...
        customSuite = OpticModel().shot(suiteType: .Face)

        //: if filterSuite == nil {
        if filterSuite == nil {
            //: filterSuite  = STFilterEntiretycostModel.init()
            filterSuite = OpticModel()
        }
    }
    ...
    func visitorSession() {
        ...
        if #available(iOS 11.0, *) {
            ...
            try? data?.write(to: URL(fileURLWithPath: path))
            //: print("TarImmediatelyCacheCost storeRst ")
            //: } else {
        } else {
    ...
    func cacheAfter() -> String {
        //: let userId = 0
        let userId = 0

        //: let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent("App/TarImmediatelyCacheCost")
        let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent((String(m_mineText) + String(appExploreIdent.suffix(6))))
    ...
    func handPieDog(curBeautySuite: AboardMagnitudeervalLiteral) {
    ...
    func discourseModel(model: PerformanceThen) {
    ...
    public func abstract() -> PerformanceThen? {
    ...
    func momentAcross(rawValue: Float) -> Float {
    ...
    func answerInsert() {
        ...
        let orignArray = PerformanceThen().withRemark(categoryType: .Filter)
        ...
        if orignArray.count > 0 {
            ...
            self.filterModelArray = NSMutableArray(array: model?.subFilterItemsArr ?? [PerformanceThen].init()) as! [PerformanceThen]
        }
    }
}
