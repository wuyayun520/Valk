...
/*: "gifts" :*/
fileprivate let appStarStr:String = "gifnames"

/*: "vipGifts" :*/
fileprivate let appActualMessage:[Character] = ["v","i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let mainReportFormat:String = "packGiftsfit security answer"

/*: "giftSendItem" :*/
fileprivate let main_medalMsg:String = "giftSnearby only volume focus advanced"
fileprivate let showGeneralData:String = "endItemtable main"
...
// __CLOSE_PRINT__
//
//  FriendshipBoltGiftManager.swift
//  AbroadTalking
//
...
class FriendshipBoltGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [ComprehensiveTextConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = PipSqueakEquatable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = FriendshipBoltGiftManager()
...
extension FriendshipBoltGiftManager {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func funcByCompletion(type: PipSqueakEquatable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        FriendshipBoltGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[ComprehensiveTextConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            FriendshipBoltGiftManager.share.soundCompletion(completion: { isSucceed in
            ...
            FriendshipBoltGiftManager.share.balanceCity(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            FriendshipBoltGiftManager.share.simultaneously(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            FriendshipBoltGiftManager.share.giftPrepare(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func giftPrepare(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = FriendshipBoltGiftManager.share.giftType
        //: LifeReactiveCompatible.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        LifeReactiveCompatible.shadow(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [ComprehensiveTextConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(appStarStr.replacingOccurrences(of: "name", with: "t"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BoltHandyJSON>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[ComprehensiveTextConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(appActualMessage))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BoltHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ComprehensiveTextConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(mainReportFormat.prefix(9)))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BoltHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ComprehensiveTextConvertible.Bags] = bagsArr
                }
                ...
                var firstModel = GiftRoofHandyJSON()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(main_medalMsg.prefix(5)) + String(showGeneralData.prefix(7)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<GiftRoofHandyJSON>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func soundCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = FriendshipBoltGiftManager.share.giftType
        //: LifeReactiveCompatible.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        LifeReactiveCompatible.highlight(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func heel(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: LifeReactiveCompatible.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        LifeReactiveCompatible.croixDeGuerre(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<DamnHandyJSON>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func balanceCity(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = PipSqueakEquatable.live
        //: LifeReactiveCompatible.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        LifeReactiveCompatible.procure(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func simultaneously(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = PipSqueakEquatable.party
        //: LifeReactiveCompatible.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        LifeReactiveCompatible.replaceWhen(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func tillCur(perPageCount: Int, dataType: ComprehensiveTextConvertible) -> Int {
    ...
    func publisher(dataType: ComprehensiveTextConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func fleet() -> [Any] {
        ...
        if FriendshipBoltGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if FriendshipBoltGiftManager.share.giftType == .live || FriendshipBoltGiftManager.share.giftType == .party {
    ...
    func trailheadArr(dataType: ComprehensiveTextConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension FriendshipBoltGiftManager {
    ...
    private func remoteNotification() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(usedClose),
    ...
    @objc private func usedClose() {
        //: gift_clearMemoryCache(showRed: true)
        diskCommit(showRed: true)
    }
}
...
extension FriendshipBoltGiftManager {
    ...
    func diskCommit(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        FriendshipBoltGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
