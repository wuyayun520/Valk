...
/*: "uid" :*/
fileprivate let k_chaseName:String = "uformal"

/*: "onlineStatus" :*/
fileprivate let appCommitSoundTransformText:String = "filterlin"

/*: "isNewUser" :*/
fileprivate let kCosyBoundaryId:String = "fore house install mainisNewUs"
fileprivate let main_farPath:[Character] = ["e","r"]

/*: "isTPAuth" :*/
fileprivate let mPositionId:String = "stand"
fileprivate let dream_readerPath:String = "sTPAuthclearly bond"

/*: "totalIntimate" :*/
fileprivate let mRationalKey:String = "TOTAL"
fileprivate let main_commandStr:[Character] = ["e"]

/*: "userStatus" :*/
fileprivate let mExceptValue:String = "uopeningr"
fileprivate let user_retId:String = "cookie"

/*: "intimate" :*/
fileprivate let userDefenseFormat:[Character] = ["i","n","t","i","m","a"]
fileprivate let mainRidExistUrl:[Character] = ["t","e"]
...
enum ButterflyConstantTarget: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class EveryListManager {
    ...
    private var moreMsgModel: SayPreviouslyReactiveCompatible = //: return SayPreviouslyReactiveCompatible.getPlaceholderModel(chatType: .moreMsg)
        .tar(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func finish() {
        //: self.officialArr = AmongObjectManager.shared.getOfficialList()
        self.officialArr = AmongObjectManager.shared.viaBy()
        //: self.topArr = AmongObjectManager.shared.topConvList
        self.topArr = AmongObjectManager.shared.topConvList
        //: self.normalArr = AmongObjectManager.shared.norConvList
        self.normalArr = AmongObjectManager.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        spirogram()
    ...
    func romm(completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        guard self.isHalf == false else { return }
        //: AmongObjectManager.shared.getChatRoomData { succeed in
        AmongObjectManager.shared.withFollow { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = AmongObjectManager.shared.chatRoomList
                self.chatRoomArr = AmongObjectManager.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func illustration(indexPath: IndexPath) -> SayPreviouslyReactiveCompatible? {
    ...
    func calledBirth(model: SayPreviouslyReactiveCompatible?) -> SayPreviouslyReactiveCompatible? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: RelatedTransformable? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: RelatedTransformable? = StateThen.goldId(targetId: targetId)
    ...
    func minFinish(conversationList: [SayPreviouslyReactiveCompatible], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        StateThen.cutOut(with: tempArr) { _, error in
    ...
    func modelAction(conversationModel: SayPreviouslyReactiveCompatible) {
    ...
    func belowBubble(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: AmongObjectManager.shared.func__getConversationList { arr, leftMore in
        AmongObjectManager.shared.engine { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [SayPreviouslyReactiveCompatible] = //: return Array<SayPreviouslyReactiveCompatible>()
        .init()
    //: }()
}
...
extension EveryListManager {
    ...
    func spirogram() {
        //: self.moreMsgArr = AmongObjectManager.shared.moreMsgList
        self.moreMsgArr = AmongObjectManager.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func existEnable(indexPath: IndexPath) -> SayPreviouslyReactiveCompatible? {
    ...
    func detailOwner() -> SayPreviouslyReactiveCompatible {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        spirogram()
        ...
        moreMsgModel.unreadCount = sumerrupt()
...
extension EveryListManager {
    ...
    func privacyCompletion(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        StateThen.meCompletion(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(k_chaseName.replacingOccurrences(of: "formal", with: "id"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(appCommitSoundTransformText.replacingOccurrences(of: "filter", with: "on") + "eStatus")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(kCosyBoundaryId.suffix(7)) + String(main_farPath))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(mPositionId.replacingOccurrences(of: "stand", with: "i") + String(dream_readerPath.prefix(7)))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(mRationalKey.lowercased() + "Intimat" + String(main_commandStr))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(mExceptValue.replacingOccurrences(of: "opening", with: "se") + "Statu" + user_retId.replacingOccurrences(of: "cookie", with: "s"))].int ?? 1
                    ...
                    self.okKind(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func courseOfStudyBot(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(k_chaseName.replacingOccurrences(of: "formal", with: "id"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(appCommitSoundTransformText.replacingOccurrences(of: "filter", with: "on") + "eStatus")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(kCosyBoundaryId.suffix(7)) + String(main_farPath))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(mExceptValue.replacingOccurrences(of: "opening", with: "se") + "Statu" + user_retId.replacingOccurrences(of: "cookie", with: "s"))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((mExceptValue.replacingOccurrences(of: "opening", with: "se") + "Statu" + user_retId.replacingOccurrences(of: "cookie", with: "s"))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension EveryListManager {
    ...
    func tin() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in AmongObjectManager.shared.norConvList {
        for model in AmongObjectManager.shared.norConvList {
            ...
            if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
                ...
                let userModel = self.calledBirth(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in AmongObjectManager.shared.topConvList {
        for model in AmongObjectManager.shared.topConvList {
            //: if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= AppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func unAgency(convModel: SayPreviouslyReactiveCompatible, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func currencyEdit(indexPath: IndexPath) -> SayPreviouslyReactiveCompatible? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func okKind(uid: String, intimate: Int) {
        ...
        StateThen.winterize(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func modifyCreateer(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(k_chaseName.replacingOccurrences(of: "formal", with: "id"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(userDefenseFormat) + String(mainRidExistUrl))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
