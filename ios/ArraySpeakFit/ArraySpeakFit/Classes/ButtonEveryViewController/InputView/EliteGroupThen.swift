...
fileprivate let m_verbalMessage:[UInt8] = [0xf1,0xf6,0xf1,0xec,0xb0,0xfb,0xf7,0xfc,0xfd,0xea,0xa2,0xb1,0xb8,0xf0,0xf9,0xeb,0xb8,0xf6,0xf7,0xec,0xb8,0xfa,0xfd,0xfd,0xf6,0xb8,0xf1,0xf5,0xe8,0xf4,0xfd,0xf5,0xfd,0xf6,0xec,0xfd,0xfc]

private func galleryBolt(deadline num: UInt8) -> UInt8 {
    return num ^ 152
}

/*: "icon_coinxxx" :*/
fileprivate let noti_entryAgreePath:String = "icon_cdream wed assert tail just"
fileprivate let dream_nearbyChopUrl:String = "oinmultiplemultiplemultiple"

/*: "icon_gift_num_bg" :*/
fileprivate let k_propertyFormat:[Character] = ["i","c","o","n","_","g","i","f","t"]
fileprivate let dreamGenderUrl:String = "_num_bgdisable plain"
...
class EliteGroupThen: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = BoltHandyJSON()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: m_verbalMessage.map{galleryBolt(deadline: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.concern(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: GoddamnThen = {
        //: let btn = TalkingButton.init()
        let btn = GoddamnThen()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.concern(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.verbalDescription(name: (String(noti_entryAgreePath.prefix(6)) + dream_nearbyChopUrl.replacingOccurrences(of: "multiple", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.verbalDescription(name: (String(k_propertyFormat) + String(dreamGenderUrl.prefix(7))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.concern(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension EliteGroupThen {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func my(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.storageEngine().cgColor
    ...
    func supply(currenmodel: BoltHandyJSON, giftType: ComprehensiveTextConvertible) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.simultaneouslyElsewhere(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.simultaneouslyElsewhere(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.concern(type: .Regular, fontSize: 11),
...
extension EliteGroupThen {
    ...
    func package() {
        ...
        bgView.backgroundColor = UIColor.airFlow()
