...
fileprivate let dream_okTitle:String = "设\u{7f6e}普通状态"
fileprivate let m_cropPullClubFormat:String = "时 请\u{4f7f}用RGB"
fileprivate let app_bindPath:String = "空间\u{7684}\u{989c}色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let showLocalPath:[Character] = ["\u{8bbe}","\u{7f6e}","选","中","状","\u{6001}","\u{7684}","文","字","颜"]
fileprivate let dreamMMutualMessage:String = "色时 "
fileprivate let mItWheelFormat:[Character] = ["请","\u{4f7f}","用","R","G","B","空","间","的","颜","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let mainDirectionFormat:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

/*: "设置的下标不合法!!" :*/
fileprivate let show_careKey:[Character] = ["设","置","\u{7684}"]
fileprivate let showPhotoData:[Character] = ["下","标","\u{4e0d}","合","法","!","!"]

/*: "Bags" :*/
fileprivate let noti_countrySelectName:[Character] = ["B","a","g","s"]
...
// __CLOSE_PRINT__
//
//  PresenceReactiveCompatible.swift
//  ZEScollViewPage
//
...
open class PresenceReactiveCompatible: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: WrittenMaterialBoltWaySegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.searchRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((dream_okTitle + "的文字颜色" + m_cropPullClubFormat + app_bindPath))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.searchRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(showLocalPath) + dreamMMutualMessage.capitalized + String(mItWheelFormat)))
        }
    ...
    fileprivate func searchRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: WrittenMaterialBoltWaySegmentStyle, titles: [String]) {
    ...
    @objc func pause(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? FourthLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDirectionFormat.map{$0^189}, encoding: .utf8)!)
    }
}
...
public extension PresenceReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func forte(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(show_careKey) + String(showPhotoData)))
...
//: extension ScrollSegmentView {
extension PresenceReactiveCompatible {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func motionConnect() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = FourthLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.pause(_:)))
            ...
            if title == (String(noti_countrySelectName)).localized {
    ...
    fileprivate func sign() {
        ...
        if let firstLabel = labelsArray[0] as? FourthLabel {
    ...
    fileprivate func everyCounterruptCover() {
    ...
    public func makeweight(_ currentIndex: Int) {
...
open class FourthLabel: UILabel {
    /// 用来记录当前label的缩放比例
