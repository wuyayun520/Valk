...
// __CLOSE_PRINT__
//
//  ComparableStuffReactiveCompatible.swift
//  AbroadTalking
//
...
class ComparableStuffReactiveCompatible: UICollectionViewFlowLayout {

    //: var sectionArr = [GiftDataType]()  
    var sectionArr = [ComprehensiveTextConvertible]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(user_iconQuantityUrl/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(kGiftIdent-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == ComprehensiveTextConvertible.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == ComprehensiveTextConvertible.Bags.rawValue {
        ...
        let itemWidth = Int(kGiftIdent-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(kGiftIdent-10) + pageCol * itemWidth
