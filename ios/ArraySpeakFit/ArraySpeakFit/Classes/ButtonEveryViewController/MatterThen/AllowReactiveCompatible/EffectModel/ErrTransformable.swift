...
/*: "num" :*/
fileprivate let dreamSaveDitClearStr:[Character] = ["n","u","m"]
...
@objc public enum LackHashable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ErrTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func giveaway(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct LaborModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(dreamSaveDitClearStr))
    }
}
