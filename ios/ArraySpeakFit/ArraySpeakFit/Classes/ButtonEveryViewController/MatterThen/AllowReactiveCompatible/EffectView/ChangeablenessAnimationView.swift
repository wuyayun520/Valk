...
fileprivate let app_multiMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "%@.mp4" :*/
fileprivate let userPairMessage:String = "ta"
fileprivate let dream_sunMessage:[Character] = ["@",".","m","p","4"]

/*: "http" :*/
fileprivate let m_invitationName:String = "abouttp"
...
// __CLOSE_PRINT__
//
//  ChangeablenessAnimationView.swift
//  AbroadTalking
//
...
class ChangeablenessAnimationView: LaborView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_multiMessage.reversed(), encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(earlierTap))
    ...
    override func tillShould() {
        ...
        let path = RelatedAnimatTool.shared.trademarkDatabaseModel(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? TarMsgModel())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (userPairMessage.replacingOccurrences(of: "ta", with: "%") + String(dream_sunMessage)), path)
            ...
            self.delegate?.lateView(effectItemView: self)
        }
    }

    //: override func stopAnimating() {
    override func rowCapture() {
        //: self.vapPlayerView?.stopHWDMP4()
        self.vapPlayerView?.stopHWDMP4()
    }

    //: override func cleanAnimating() {
    override func thanAnimating() {
    ...
    @objc func earlierTap() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension ChangeablenessAnimationView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((m_invitationName.replacingOccurrences(of: "about", with: "ht"))) {
