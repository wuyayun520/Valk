...
fileprivate let main_jackYieldData:[UInt8] = [0x11,0x16,0x11,0xc,0x50,0x1b,0x17,0x1c,0x1d,0xa,0x42,0x51,0x58,0x10,0x19,0xb,0x58,0x16,0x17,0xc,0x58,0x1a,0x1d,0x1d,0x16,0x58,0x11,0x15,0x8,0x14,0x1d,0x15,0x1d,0x16,0xc,0x1d,0x1c]

private func reverseRet(take num: UInt8) -> UInt8 {
    return num ^ 120
}

/*: "move" :*/
fileprivate let dream_arcStr:String = "mmindve"
...
// __CLOSE_PRINT__
//
//  SayDigitalRecognizerDelegate.swift
//  AbroadTalking
//
...
class SayDigitalRecognizerDelegate: UIView {
    ...
    var danumuMsgArray: [ModelType] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [RicochetReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ModelType?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_jackYieldData.map{reverseRet(take: $0)}, encoding: .utf8)!)
    }
    ...
    func belowTotal(_ danumuMsgModel: ModelType?) {
    ...
    @objc func actionImage(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(actionImage(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension SayDigitalRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    func lip(for danumuMsgModel: ModelType?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = RicochetReactiveCompatible(model: danumuMsgModel)
        ...
        var frame = CGRect(x: kGiftIdent, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (dream_arcStr.replacingOccurrences(of: "mind", with: "o")))
...
//    }
    ...
    func valueObjectChannel(_ isFirst: Bool) {
...
extension SayDigitalRecognizerDelegate {
    // 添加视图
