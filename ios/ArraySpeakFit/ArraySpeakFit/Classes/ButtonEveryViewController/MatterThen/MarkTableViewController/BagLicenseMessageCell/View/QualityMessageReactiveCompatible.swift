...
fileprivate let dreamKingData:[UInt8] = [0x3d,0x42,0x3d,0x48,0xfc,0x37,0x43,0x38,0x39,0x46,0xe,0xfd,0xf4,0x3c,0x35,0x47,0xf4,0x42,0x43,0x48,0xf4,0x36,0x39,0x39,0x42,0xf4,0x3d,0x41,0x44,0x40,0x39,0x41,0x39,0x42,0x48,0x39,0x38]

fileprivate func upperBolt(operation num: UInt8) -> UInt8 {
...
fileprivate let mMultipleFormat:String = "icon_inmate child abstract till should"
fileprivate let notiSharedValue:String = "nner_cosy enjoy movie world tar"
fileprivate let userProcedureAmazingTwoMsg:[Character] = ["s","u","b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let mainTextureSomeoneKey:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u"]
fileprivate let showSolarMessage:String = "btpartya"

/*: "bannerList" :*/
fileprivate let kControlMsg:[Character] = ["b","a","n","n","e","r"]
fileprivate let userEnableCapabilityMessage:String = "Listfade third nature"

/*: "JXBannerCellID" :*/
fileprivate let dream_frontPath:[Character] = ["J","X","B","a","n","n","e","r"]
fileprivate let appTenderUrl:String = "thanks civic week thanks shadowCellID"

/*: "#FFFFFF" :*/
fileprivate let user_termsYearStr:[Character] = ["#","F","F","F","F","F","F"]
...
protocol DelegateThen: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func identification(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class QualityMessageReactiveCompatible: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: DelegateThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamKingData.map{upperBolt(operation: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [SocietalHandyJSON] = //: return Array<SocietalHandyJSON>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(mMultipleFormat.prefix(5)) + "chats_ba" + String(notiSharedValue.prefix(5)) + String(userProcedureAmazingTwoMsg))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.verbalDescription(name: (String(mainTextureSomeoneKey) + showSolarMessage.replacingOccurrences(of: "party", with: "r") + "ct_pre")), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(maturityDate), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension QualityMessageReactiveCompatible {
    /// banner
    //: func reqBanner() {
    func extendedFamily() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        BubbleRequestManager().progressive(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(kControlMsg) + String(userEnableCapabilityMessage.prefix(4)))] ?? []).arrayValue
...
extension QualityMessageReactiveCompatible {
    ...
    @objc private func maturityDate() {
        ...
        self.delegate?.identification(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func heart(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension QualityMessageReactiveCompatible: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(dream_frontPath) + String(appTenderUrl.suffix(6))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.beyondFore(urlStr: url, placeImg: UIImage.damage())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(user_termsYearStr)))!
