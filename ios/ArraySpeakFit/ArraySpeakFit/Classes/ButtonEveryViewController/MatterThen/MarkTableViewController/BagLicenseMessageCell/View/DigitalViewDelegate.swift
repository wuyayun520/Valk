...
fileprivate let show_speculateHusbandSendFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#AAAAAA" :*/
fileprivate let dream_bulletMessage:[Character] = ["#","A","A","A","A","A","A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let user_neverName:[Character] = ["b","t","n","_","c","h"]
fileprivate let dream_resMessage:String = "afinancialet"
fileprivate let showItsFormat:String = "udirectorchoice"

/*: "btn_chatsettings_choiced" :*/
fileprivate let showSubjectMsg:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t"]
fileprivate let user_momentResignFormat:String = "about me problem added incidentings_c"

/*: "Top" :*/
fileprivate let notiReloadlyValue:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let appEveningData:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let m_methodUrl:[Character] = ["C"]
fileprivate let show_distanceKey:String = "arealizecel"
...
class DigitalViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_speculateHusbandSendFormat.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + user_sizeValue
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: user_allMessage - height, width: kGiftIdent, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.shRadii(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.colouringDramatically()
    ...
    private lazy var textView: YYTextView = {
        ...
        tv.textColor = UIColor.telecom()
        //: tv.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        tv.font = UIFont.concern(type: .Regular, fontSize: 16)
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(dream_bulletMessage)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: GoddamnThen = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(user_neverName) + dream_resMessage.replacingOccurrences(of: "financial", with: "ts") + "tings_" + showItsFormat.replacingOccurrences(of: "director", with: "n"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.verbalDescription(name: (String(showSubjectMsg) + String(user_momentResignFormat.suffix(6)) + "hoiced")), for: .selected)
        ...
        btn.setTitle((String(notiReloadlyValue)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(simultaneouslyOf), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(appEveningData)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(viewHoldfast), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(m_methodUrl) + show_distanceKey.replacingOccurrences(of: "realize", with: "n")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(cancelWithoutClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension DigitalViewDelegate: YYTextViewDelegate {
...
extension DigitalViewDelegate {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func viewHoldfast() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: LifeReactiveCompatible.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        LifeReactiveCompatible.noticePhotoOffce(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    @objc private func simultaneouslyOf() {
    ...
    @objc private func cancelWithoutClick() {
        //: dismiss()
        pop()
    }
    ...
    func appropriate(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
