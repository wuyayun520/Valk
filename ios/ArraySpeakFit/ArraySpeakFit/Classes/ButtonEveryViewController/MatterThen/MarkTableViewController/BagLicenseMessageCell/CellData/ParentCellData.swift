...
fileprivate let k_cookieUrl:[UInt8] = [0xd9,0xde,0xd9,0xc4,0x98,0xd3,0xdf,0xd4,0xd5,0xc2,0x8a,0x99,0x90,0xd8,0xd1,0xc3,0x90,0xde,0xdf,0xc4,0x90,0xd2,0xd5,0xd5,0xde,0x90,0xd9,0xdd,0xc0,0xdc,0xd5,0xdd,0xd5,0xde,0xc4,0xd5,0xd4]

/*: "[未知消息]" :*/
fileprivate let show_shouldMessage:[Character] = ["[","\u{672a}","知","消","息","]"]

/*: "extra" :*/
fileprivate let k_aliveDrawingMsg:[Character] = ["e","x","t","r","a"]

/*: "#0084FF" :*/
fileprivate let mOrientationFinishPath:String = "engage fragment as lat chemistry#0084FF"

/*: "#FFEE4B" :*/
fileprivate let show_textMsg:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let notiFireLapKey:String = "<at>@[way successful"
fileprivate let dreamBeautifulMsg:String = "+?</at>sale point cost automatically in"

/*: "jumpKey" :*/
fileprivate let show_analyzeDealName:[Character] = ["j","u","m","p","K","e","y"]

/*: "user" :*/
fileprivate let main_fusionId:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let dreamRefuseValue:[UInt8] = [0xe3,0xd7,0xd2]

fileprivate func programDrown(lip num: UInt8) -> UInt8 {
...
/*: "#16D073" :*/
fileprivate let dreamAverageMsg:[Character] = ["#","1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let main_beatData:String = "#Fadvertising global"
fileprivate let showPhoneIdent:String = "956data"
...
//: @objcMembers public class ParentCellData: TalkingChatMsgBaseCellData {
@objcMembers public class ParentCellData: OthersAlterCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_cookieUrl.map{$0^176}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: DramaticallyModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (String(show_shouldMessage))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = exOfEarly(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func exOfEarly(msgInfoModel: QualityModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(k_aliveDrawingMsg))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.telecom() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(mOrientationFinishPath.suffix(7))))! : UIColor(hex: (String(show_textMsg)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(notiFireLapKey.prefix(6)) + "\\S\\s]" + String(dreamBeautifulMsg.prefix(7))), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(show_analyzeDealName))].stringValue == (String(main_fusionId)) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: dreamRefuseValue.map{programDrown(lip: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: dreamRefuseValue.map{programDrown(lip: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func promptCapture() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(dreamAverageMsg)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(main_beatData.prefix(2)) + showPhoneIdent.replacingOccurrences(of: "data", with: "44")))!
        }
