...
fileprivate let app_assertMessage:[UInt8] = [0x9c,0x9b,0x9c,0x81,0xdd,0x96,0x9a,0x91,0x90,0x87,0xcf,0xdc,0xd5,0x9d,0x94,0x86,0xd5,0x9b,0x9a,0x81,0xd5,0x97,0x90,0x90,0x9b,0xd5,0x9c,0x98,0x85,0x99,0x90,0x98,0x90,0x9b,0x81,0x90,0x91]

private func bagStat(s num: UInt8) -> UInt8 {
    return num ^ 245
}

/*: "icon_lounge" :*/
fileprivate let user_ratingBrigStr:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_talk_points" :*/
fileprivate let user_importantMsg:String = "icofragment"
fileprivate let k_contrastMsg:String = "k_pointsmain visual"

/*: "C6BDFF" :*/
fileprivate let main_standardName:[Character] = ["C","6","B","D","F","F"]

/*: "#BBBBBB" :*/
fileprivate let main_sureMsg:String = "#BBBBpretty limited dynamic processor am"
fileprivate let kSectionText:String = "starstar"

/*: "Reply to get points~" :*/
fileprivate let showTimeNobodyMsg:String = "Reply popular enthusiasm late we news"
fileprivate let appSlightValue:String = "et poinsend interested"
fileprivate let app_sortKey:[Character] = ["t","s","~"]

/*: "bth_unread_nor" :*/
fileprivate let notiMakerId:String = "picture back pack nightbth_un"
fileprivate let m_maximumPlantStr:String = "yourself ladder convert listen action_nor"

/*: "icon_male_default" :*/
fileprivate let mainFleshValue:[UInt8] = [0x11,0x1b,0x17,0x16,0x27,0x15,0x19,0x14,0x1d,0x27,0x1c,0x1d,0x1e,0x19,0xd,0x14,0xc]

/*: "icon_female_default" :*/
fileprivate let userAlongPath:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d","e","f","a","u","l","t"]

/*: "+%@ points" :*/
fileprivate let notiAmKey:String = "+%@"
fileprivate let user_judgeTitle:String = "observer across pick administrative interval points"

/*: "bth_read_pre" :*/
fileprivate let app_beanStr:[Character] = ["b","t","h","_","r","e","a","d","_","p","r","e"]
...
// __CLOSE_PRINT__
//
//  CapacityCellReactiveCompatible.swift
//  AbroadTalking
//
...
//: class TalkingChatBaseMsgCell: FourthGuideViewCell {
class CapacityCellReactiveCompatible: FourthGuideViewCell {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: OthersAlterCellData?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        super.init(style: style, reuseIdentifier: reuseIdentifier)
        //: self.setupSubviews()
        self.tensionSubviews()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: app_assertMessage.map{bagStat(s: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(user_ratingBrigStr)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.equitySize(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.verbalDescription(name: (user_importantMsg.replacingOccurrences(of: "fragment", with: "n") + "_tal" + String(k_contrastMsg.prefix(8))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(main_standardName)))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.duringWindowExtreme(fontSize: 14)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(main_sureMsg.prefix(5)) + kSectionText.replacingOccurrences(of: "star", with: "B")))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.duringWindowExtreme(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(showTimeNobodyMsg.prefix(6)) + "to g" + String(appSlightValue.prefix(7)) + String(app_sortKey)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.verbalDescription(name: (String(notiMakerId.suffix(6)) + "read" + String(m_maximumPlantStr.suffix(4))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension CapacityCellReactiveCompatible {
    ...
    override func tip(with data: AccountingDataThen) {
        ...
        self.cellData = data as? OthersAlterCellData
        ...
        self.nameLabel.textColor = loungePlus ? .atSituationRaw() : .componentHighlight()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.equitySize(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(AlongsideDigitalTableConvertible.male.rawValue) ? String(bytes: mainFleshValue.map{$0^120}, encoding: .utf8)! : (String(userAlongPath))
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.verbalDescription(name: imgStr), for: .normal)
        ...
        self.avatarView.simultaneouslyElsewhere(urlStr: userModel.headPic)
        ...
        self.iconBorder.setFromImaginationImage(urlStr: userModel.headPicFrame)
        ...
        self.coinIconImg.isHidden = !(isShowCoin && AppManager.share.appStatus == WrinkleSendable.normal.rawValue)
        ...
        self.coinLabel.text = (notiAmKey.capitalized + String(user_judgeTitle.suffix(7))).launchRid(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if LifeDigitalThen.array(newData.direction,
        ...
        if LifeDigitalThen.everyTap(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(notiMakerId.suffix(6)) + "read" + String(m_maximumPlantStr.suffix(4)))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if LifeDigitalThen.comprehend(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(app_beanStr))
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.verbalDescription(name: readImgStr)
