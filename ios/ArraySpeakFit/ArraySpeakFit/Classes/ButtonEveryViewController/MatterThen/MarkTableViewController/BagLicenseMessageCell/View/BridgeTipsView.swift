...
fileprivate let noti_afterUrl:[UInt8] = [0x9d,0x9a,0x9d,0x80,0xdc,0x97,0x9b,0x90,0x91,0x86,0xce,0xdd,0xd4,0x9c,0x95,0x87,0xd4,0x9a,0x9b,0x80,0xd4,0x96,0x91,0x91,0x9a,0xd4,0x9d,0x99,0x84,0x98,0x91,0x99,0x91,0x9a,0x80,0x91,0x90]

private func seatMerge(close num: UInt8) -> UInt8 {
    return num ^ 244
}
...
class BridgeTipsView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_afterUrl.map{seatMerge(close: $0)}, encoding: .utf8)!)
    }
...
extension BridgeTipsView {
    //: func setData() {
    func siphonServerData() {
        ...
        if AppManager.share.appUserConfigMode.quickReplyText.count > 4 {
            ...
            let getRandom = switchice(min: 0, max: AppManager.share.appUserConfigMode.quickReplyText.count - 1)
            //: for _ in 0...3 {
            for _ in 0 ... 3 {
                //: let index =  getRandom()
                let index = getRandom()
    ...
    func switchice(min: Int, max: Int) -> () -> Int {
        //: var numbers: [Int] = []
        var numbers: [Int] = []
        //: return {
        return {
    ...
    func groupDiscussion() {
        ...
        for (i, str) in quickReplyText.enumerated() {
            ...
            let rect = str.boundingRect(with: size, options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [NSAttributedString.Key.font: UIFont.concern(type: .Regular, fontSize: 13)], context: nil)
            ...
            let tap = UITapGestureRecognizer(target: self, action: #selector(sinceReload(sender:)))
    ...
    @objc func sinceReload(sender: UITapGestureRecognizer) {
        //: let lb = sender.view as? UILabel
        let lb = sender.view as? UILabel
        //: self.delegate?.seleteReplyTipsMessage(str: lb?.text ?? "")
        self.delegate?.spendStr(str: lb?.text ?? "")
    }
}
