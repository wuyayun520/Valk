...
fileprivate let notiTagFormat:[UInt8] = [0xf,0x14,0xf,0x1a,0xce,0x9,0x15,0xa,0xb,0x18,0xe0,0xcf,0xc6,0xe,0x7,0x19,0xc6,0x14,0x15,0x1a,0xc6,0x8,0xb,0xb,0x14,0xc6,0xf,0x13,0x16,0x12,0xb,0x13,0xb,0x14,0x1a,0xb,0xa]

fileprivate func depthJob(refer num: UInt8) -> UInt8 {
...
/*: "icon_chat_start" :*/
fileprivate let user_lapExpressionStr:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let appOuterKey:String = "preparation positive spectrum reint_start"

/*: "icon_chat_lock" :*/
fileprivate let notiImportantPath:String = "iguest"
fileprivate let main_preparationPastKey:[Character] = ["o","n","_","c","h","a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let dream_edgeKey:[Character] = ["i","c","o","n","_","c","h"]
fileprivate let noti_singleTitle:String = "at_bsweep tension effect split bond"

/*: "icon_chat_burned_shan" :*/
fileprivate let mainSuccessfulCloudKey:String = "multiple drag sufficient handicon_c"
fileprivate let mRackUniversalFormat:[Character] = ["h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let app_heliText:[Character] = ["A","l","r","e","a","d"]
fileprivate let showMonthId:String = "y burnedprofessional roughly shoulder bound trap"
...
class TailThen: CapacityCellReactiveCompatible {
    //: var videoData: CryOutCellData?
    var videoData: CryOutCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiTagFormat.map{depthJob(refer: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.verbalDescription(name: (String(user_lapExpressionStr) + String(appOuterKey.suffix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.verbalDescription(name: (notiImportantPath.replacingOccurrences(of: "guest", with: "c") + String(main_preparationPastKey)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: GoddamnThen = {
        ...
        imgV.setBackgroundImage(UIImage.verbalDescription(name: (String(dream_edgeKey) + String(noti_singleTitle.prefix(4)) + "urned")), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.verbalDescription(name: (String(mainSuccessfulCloudKey.suffix(6)) + String(mRackUniversalFormat))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(app_heliText) + String(showMonthId.prefix(8))).localized, for: .normal)
...
extension TailThen {
    ...
    override func tip(with data: AccountingDataThen) {
        ...
        super.tip(with: data)
        //: self.videoData = data as? CryOutCellData
        self.videoData = data as? CryOutCellData
        ...
        self.dutationLab.text = NSDate.equityTime(totalTime: duration)
        ...
        self.scope(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func scope(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.tapLevel(image, shape: 90)
                self.picImgView.image = UIImage.tapLevel(image, shape: 90)
                //: } else {
            } else {
            ...
            self.picImgView.askFor(urlStr: urlStr) { [weak self] image in
