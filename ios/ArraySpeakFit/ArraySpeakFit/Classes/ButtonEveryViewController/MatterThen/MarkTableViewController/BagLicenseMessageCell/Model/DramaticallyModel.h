...
@interface QualityModel : JSONModel
...
@property (nonatomic, strong) NonopeningModel *intimatePhoto; /// 有该字段表示为私密照片
//: @property (nonatomic, strong) AbTalkingChatMsgVideoModel *video; 
@property (nonatomic, strong) NextAdviceModel *video; /// 有该字段表示是视频消息
//: @property (nonatomic, strong) AbTalkingChatMsgCustomSendModel *customSend; 
@property (nonatomic, strong) OutsideModel *customSend; /// 保存自定义发送字段（用于失败重发）

//: @property (nonatomic, strong) WantGiftModel *wantGift; 
@property (nonatomic, strong) BugModel *wantGift; /// 想要礼物
...
@interface NonopeningModel : JSONModel
...
+ (void)storageCountimate:(NonopeningModel *)photoModel tableLevel:(NSString *)msgID;

//: + (NSMutableDictionary<NSString *, NSString *> *)loadLocalIntimatePhotoDict;
+ (NSMutableDictionary<NSString *, NSString *> *)page;
...
@interface NextAdviceModel: JSONModel
...
+ (void)sumimate:(NextAdviceModel *)videoModel attitude:(NSString *)msgID;
...
@interface OutsideModel: JSONModel
...
@interface RealizeUserModel : JSONModel
...
@interface ConvertModel : JSONModel
...
+ (void)lock:(NSString *)msgID;

//: + (NSMutableArray *)loadAskForGiftArr;
+ (NSMutableArray *)observerGravity;

//: @end
@end

//: @interface AbTalkingChatMsgQuoteModel : JSONModel
@interface RealizeHeliModel : JSONModel
...
@property (nonatomic, strong) OutlookModel *renderData;

//: - (BOOL)isQuoteMsg;
- (BOOL)brightMsg;

//: - (CGSize)quoteSize:(NSInteger)maxWidth;
- (CGSize)up:(NSInteger)maxWidth;
...
@interface OutlookModel : JSONModel
