...
fileprivate let app_beginKey:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

private func complyMissing(share num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "#FA9D33" :*/
fileprivate let app_checkData:String = "#FA9D33combined communicate policy"

/*: "#343338" :*/
fileprivate let mainPrepareWaitCalledKey:String = "#343338"
...
class LagniappeViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_beginKey.map{complyMissing(share: $0)}, encoding: .utf8)!)
    }
...
extension LagniappeViewCell {
    /// 刷新cell
    /// - Parameters:
    ///   - position: 麦位
    ///   - model: 麦位数据
    //: func refreshIcon(position: Int, model: TalkingMikeListItemModel) {
    func audienceRepresentation(position: Int, model: PerformanceMeasurable) {
        //: guard model.uid > 0, String(model.uid) != AppManager.share.loginUid else {
        guard model.uid > 0, String(model.uid) != AppManager.share.loginUid else {
        ...
        let color = model.needGift ? UIColor(hex: (String(app_checkData.prefix(7)))) : UIColor(hex: (mainPrepareWaitCalledKey.capitalized))
