...
fileprivate let noti_decreaseIdent:[UInt8] = [0xb8,0xbd,0xb8,0xc3,0x77,0xb2,0xbe,0xb3,0xb4,0xc1,0x89,0x78,0x6f,0xb7,0xb0,0xc2,0x6f,0xbd,0xbe,0xc3,0x6f,0xb1,0xb4,0xb4,0xbd,0x6f,0xb8,0xbc,0xbf,0xbb,0xb4,0xbc,0xb4,0xbd,0xc3,0xb4,0xb3]

fileprivate func airRefuse(warm num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let user_redLaterForeheadUrl:String = "bth_livsubject amp appear display"
fileprivate let mainPerfectFormat:[Character] = ["e","_","f","o","l","l","o","w","_","p","r","e"]

/*: "btn_live_closed_nor" :*/
fileprivate let notiOfKey:String = "btn_livawake unless"
fileprivate let mainTransportPath:[Character] = ["e","_","c","l","o"]
fileprivate let noti_birthdayHistoryUrl:String = "sed_norresolution array success enough cut"

/*: "removeAttentionUid" :*/
fileprivate let mainAmongId:String = "REMO"
fileprivate let show_monitorName:String = "nrefrigeratoro"
fileprivate let mEnhanceMsg:String = "director amazingnUid"

/*: "attentionUid" :*/
fileprivate let userNeatFormat:[Character] = ["a","t","t","e","n"]
fileprivate let kFeverIdent:String = "T"
fileprivate let user_averageId:String = "or template headionUid"

/*: "source" :*/
fileprivate let notiActivityKickMenuMsg:String = "sourcskull"

/*: "num" :*/
fileprivate let mTickStr:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let m_serviceId:[Character] = ["U","I","D",":"]
...
class PrecipitateThen: UIView {
    ...
    private var partyModel = MoveModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_decreaseIdent.map{airRefuse(warm: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(propertyClick), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(user_redLaterForeheadUrl.prefix(7)) + String(mainPerfectFormat))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(geneticProfiling), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.verbalDescription(name: (String(notiOfKey.prefix(7)) + String(mainTransportPath) + String(noti_birthdayHistoryUrl.prefix(7)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(miniskirt), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineListView: PhysicalListView = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = PhysicalListView()
...
extension PrecipitateThen {
    ...
    @objc func sortToCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(mainAmongId.lowercased() + "veAtte" + show_monitorName.replacingOccurrences(of: "refrigerator", with: "ti") + String(mEnhanceMsg.suffix(4)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(userNeatFormat) + kFeverIdent.lowercased() + String(user_averageId.suffix(6)))] = uid
            //: dict["source"] = "2"
            dict[(notiActivityKickMenuMsg.replacingOccurrences(of: "skull", with: "e"))] = "2"
        }
        //: LifeReactiveCompatible.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension PrecipitateThen {
    ...
    @objc private func cosMagnitude() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        JumpVoiceReqTool.pingMagnitudeParent(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(mTickStr))].intValue
    ...
    private func citySum() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(cosMagnitude), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension PrecipitateThen {
    ...
    func nooksAndCranniesView() {
        //: partyModel = TalkingVoiceRoomManager.shared().partyModel
        partyModel = StuffLicenseReactiveCompatible.iconPoint().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(m_serviceId)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.beyondFore(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.salt(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == AppManager.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == AppManager.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            program(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            program(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        citySum()
    ...
    @objc private func propertyClick() {
    ...
    @objc private func geneticProfiling() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        sortToCompletion(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.program(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }

    /// 最小化按钮点击事件
    //: @objc private func miniButtonClick() {
    @objc private func miniskirt() {
...
extension PrecipitateThen {
    ...
    private func moment() {
        ...
        frame = CGRect(x: 0, y: 0, width: kGiftIdent, height: dreamNoSampleValue + actualWidth(w: 43))
