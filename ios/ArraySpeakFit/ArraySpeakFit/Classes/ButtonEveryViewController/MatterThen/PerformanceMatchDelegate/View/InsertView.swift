...
fileprivate let showHerLaunchValue:[UInt8] = [0xc5,0xc2,0xc5,0xd8,0x84,0xcf,0xc3,0xc8,0xc9,0xde,0x96,0x85,0x8c,0xc4,0xcd,0xdf,0x8c,0xc2,0xc3,0xd8,0x8c,0xce,0xc9,0xc9,0xc2,0x8c,0xc5,0xc1,0xdc,0xc0,0xc9,0xc1,0xc9,0xc2,0xd8,0xc9,0xc8]

private func softlyPost(pair num: UInt8) -> UInt8 {
    return num ^ 172
}
...
@objc protocol MatterViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func overRenderClear()

    //: @objc optional func endScroll()
    @objc optional func beforeDit()
}
...
class InsertView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(belowStorage))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showHerLaunchValue.map{softlyPost(pair: $0)}, encoding: .utf8)!)
    }
...
extension InsertView {
    ...
    func queryContent() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(timerExit), userInfo: nil, repeats: true)
    }
    ...
    @objc func timerExit() {
        ...
        putDoingTheLineTillUndersurface()
    }

    //: @objc func tapAction() {
    @objc func belowStorage() {
...
extension InsertView {
    ...
    func displace(msgModel: LicenseTailTransformable?) {
    ...
    func putDoingTheLineTillUndersurface() {
        // 处于爬楼状态更新更多按钮
        //: if !self.inPending {
        if !self.inPending {
    ...
    func ignore() {
        ...
        if AllHeight > mainMicId {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < mainMicId {
                ...
                self.msgTableView.height = mainMicId
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = mainMicId - AllHeight
        ...
        quickener(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func quickener(animated: Bool) {
...
extension InsertView: UITableViewDelegate, UITableViewDataSource {
    ...
    func smartComplete(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.beforeDit?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(showMeKey)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = AgeRelatedReactiveCompatible.springPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension InsertView {
    ...
    func seekSubviews() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: user_storageValue, height: mainMicId)
    }
}
