...
fileprivate let mAmSubMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class BubbleMiniView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mAmSubMessage.reversed(), encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        StackSocketManager.shared.containerView?.isHidden = false
    }
    ...
    func dismissInEnd() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= kGiftIdent * 0.5) ? -self.width : kGiftIdent + self.width
            ...
            StackSocketManager.shared.containerView?.isHidden = true
        }
    }
}
...
extension BubbleMiniView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func atTap() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.naturalEvent(sender:)))
    ...
    func libraryMessage() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.aboveEvent(sender:)))
    ...
    @objc func aboveEvent(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(kGiftIdent - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(user_allMessage - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = kGiftIdent - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = kGiftIdent
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= kGiftIdent * 0.5 {
            ...
            StackSocketManager.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func naturalEvent(sender _: UIPanGestureRecognizer) {
