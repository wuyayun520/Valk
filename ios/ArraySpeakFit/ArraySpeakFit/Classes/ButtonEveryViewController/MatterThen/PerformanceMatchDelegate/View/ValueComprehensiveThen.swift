...
fileprivate let showTaskPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#171717" :*/
fileprivate let dream_sufficientIdent:String = "#171717"

/*: "icon_cameraoff" :*/
fileprivate let mainSeriesName:String = "submitc"
fileprivate let dream_threadThirdText:String = "quit forth lip infoon_ca"
...
class ValueComprehensiveThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.optimize(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showTaskPath.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (dream_sufficientIdent.capitalized))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.verbalDescription(name: (mainSeriesName.replacingOccurrences(of: "submit", with: "i") + String(dream_threadThirdText.suffix(5)) + "meraoff"))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension ValueComprehensiveThen {
    /// 刷新视频通话背景
    /// - Parameters:
    ///   - urlStr: 背景图片
    ///   - isOpenCamera: 摄像头状态
    //: func refreshBgView(urlStr: String?, isOpenCamera: VideoCameraStatus) {
    func connectPost(urlStr: String?, isOpenCamera: ParentCameraStatus) {
...
extension ValueComprehensiveThen: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func early(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, kGiftIdent - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, user_allMessage - self.frame.size.height * 0.5)
...
extension ValueComprehensiveThen {
    ...
    private func dowery() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(overly))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(early(pan:)))
        //: pan.delegate = self
        pan.delegate = self
