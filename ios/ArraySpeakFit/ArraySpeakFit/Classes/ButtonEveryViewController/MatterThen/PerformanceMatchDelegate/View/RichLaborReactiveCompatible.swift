...
fileprivate let user_qualityValue:[UInt8] = [0x61,0x66,0x61,0x7c,0x20,0x6b,0x67,0x6c,0x6d,0x7a,0x32,0x21,0x28,0x60,0x69,0x7b,0x28,0x66,0x67,0x7c,0x28,0x6a,0x6d,0x6d,0x66,0x28,0x61,0x65,0x78,0x64,0x6d,0x65,0x6d,0x66,0x7c,0x6d,0x6c]

/*: "icon_mini_voice_end" :*/
fileprivate let app_totalCoordinateText:[UInt8] = [0x1b,0x11,0x1d,0x1c,0x2d,0x1f,0x1b,0x1c,0x1b,0x2d,0x4,0x1d,0x1b,0x11,0x17,0x2d,0x17,0x1c,0x16]

private func tailSufficient(dream num: UInt8) -> UInt8 {
    return num ^ 114
}

/*: "icon_mini_voice_calling" :*/
fileprivate let showExtentKey:String = "icon_realize rocket"
fileprivate let showKickMsg:String = "_voiceinstruction bring plastic supposed"

/*: "#F95151" :*/
fileprivate let mSimplyMessage:[Character] = ["#","F","9","5","1","5","1"]

/*: "#34C759" :*/
fileprivate let kIdentificationIdent:String = "minimum story ballot oval#34C759"

/*: "End of call" :*/
fileprivate let kImageColonUrl:[Character] = ["E","n","d"," ","o","f"," "]
fileprivate let kEngineTitle:[Character] = ["c","a","l","l"]
...
// __CLOSE_PRINT__
//
//  RichLaborReactiveCompatible.swift
//  AbroadTalking
//
...
class RichLaborReactiveCompatible: BubbleMiniView {
    // 小窗口通话状态
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_qualityValue.map{$0^8}, encoding: .utf8)!)
    }
...
extension RichLaborReactiveCompatible {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func strokeBy() -> RichLaborReactiveCompatible {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = RichLaborReactiveCompatible(frame: .zero)
...
extension RichLaborReactiveCompatible {
    ...
    private func ballotBox() {
        ...
        magnitude(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func magnitude(status: TailDumpWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: app_totalCoordinateText.map{tailSufficient(dream: $0)}, encoding: .utf8)! : (String(showExtentKey.prefix(5)) + "mini" + String(showKickMsg.prefix(6)) + "_calling")
        //: self.statusIcon.image = UIImage.BundleImageNamed(name: iconStr)
        self.statusIcon.image = UIImage.verbalDescription(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(mSimplyMessage))) : UIColor(hex: (String(kIdentificationIdent.suffix(7))))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(kImageColonUrl) + String(kEngineTitle)).localized
        }
    }
}
