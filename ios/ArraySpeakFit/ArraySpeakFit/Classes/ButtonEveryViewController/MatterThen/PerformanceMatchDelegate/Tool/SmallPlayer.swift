...
/*: "call_bgm" :*/
fileprivate let appPossibilityContent:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let m_resortMessage:[Character] = ["c","a","l","l","_","r","e","s","p"]
fileprivate let notiSpendValue:[Character] = ["o"]
fileprivate let kBehaviorMsg:String = "nse_bgmreplace color"
...
class SmallPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = QualityStrideable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func birthEdit(type: QualityStrideable) -> SmallPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = SmallPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.primrosePath()
...
extension SmallPlayer {
    ...
    private func lineOfMerchandise(notify: Notification) {
    ...
    private func soften(notify: Notification) {
    ...
    private func primrosePath() {
        ...
        var fileName = (String(appPossibilityContent))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(m_resortMessage) + String(notiSpendValue) + String(kBehaviorMsg.prefix(7)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = EffectTool.default.coOccurWith(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension SmallPlayer: TXVodPlayListener {
