...
/*: "mf/call/GetUserInfo" :*/
fileprivate let mainCareUrl:String = "indicator editormf/cal"
fileprivate let mainProfessName:String = "us out decisionl/GetU"

/*: "uid" :*/
fileprivate let mSubmitIdent:[UInt8] = [0x6f,0x63,0x5e]

fileprivate func connectFall(harassment num: UInt8) -> UInt8 {
...
/*: "TRTC 随机视频匹配中." :*/
fileprivate let userSpaceMessage:String = "evaluationRevaluationC"
fileprivate let mReplaceIdent:[Character] = [" ","随","机","视","频","匹","配","中","."]

/*: "TRTC  :*/
fileprivate let dream_teamName:String = "delay serviceTRTC "

/*: 视频通话. logId: :*/
fileprivate let mExplorationContent:String = "视频通话. lhighlight quickly education"
fileprivate let m_mineUrl:[Character] = ["o","g","I","d",":"]

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let mBullMsg:String = "max what candidTRTC 视"
fileprivate let appConnectId:String = "path skin convert creation plant log"
fileprivate let k_labelTitle:String = "Id\u{ff1a}"

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let mDownPath:String = "packRpack"
fileprivate let mFaceMessage:String = "C \u{89c6}频"
fileprivate let appExpressionStr:String = " errlower interaction exhibit carry currently"

/*: , errMsg: :*/
fileprivate let k_byFormat:String = "availability incorporate, er"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let notiReachFeedbackData:[Character] = ["T","R","T","C"," ","视","频","通","话","S","D","K","收","到","警","告","."]
fileprivate let kServerTopicIdent:String = "pile detailed share sun wa"
fileprivate let app_editText:String = "RNIN"

/*: , warningMsg: :*/
fileprivate let show_adminText:[Character] = [","," ","w","a","r"]
fileprivate let show_todayQuoteMessage:[Character] = ["n","i","n","g","M","s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let mGivingValue:String = "TcurveC"
fileprivate let app_remarkUrl:String = "\u{5165}\u{623f}\u{95f4}."

/*: , logId： :*/
fileprivate let dream_entryWordMessage:String = "talk presentation statement if, lo"

/*:  加入了房间" :*/
fileprivate let main_shotTagStr:[Character] = [" ","加","入","了","房","间"]

/*:  离开了房间" :*/
fileprivate let mProcedureFormat:[Character] = [" ","\u{79bb}","开","了","房","间"]
...
class ButtonBoltThen: NSObject {
    ...
    var remoteCameraStatus = ParentCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class BagChatManager: NSObject {
    ...
    deinit {
        ...
        self.letLooseResourceProgramEveryLast()
    }
}
...
extension BagChatManager {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func duty(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = ExecLaborReactiveCompatible()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(mainCareUrl.suffix(6)) + String(mainProfessName.suffix(6)) + "serInfo")
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: mSubmitIdent.map{connectFall(harassment: $0)}, encoding: .utf8)!: uid]
...
extension BagChatManager {
    ...
    func letLooseResourceProgramEveryLast() {
        //: destoryTRTCView()
        stackView()
    }
    ...
    func track(_ frontCamera: Bool) {
        ...
        self.domainFront(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func generalPicture(_ openCamera: Bool) {
        ...
        self.enableeOff(isOpen: openCamera)
    }
    ...
    func conclusionQuit() {
        ...
        DispatchQueue.main.async {
            ...
            self.detectBy(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
    ...
    func constraintControl() {
        // 如果没有预览先开启预览
        //: self.config.isObject = true
        self.config.isObject = true
        //: if self.config.isPreviewNow == false {
        if self.config.isPreviewNow == false {
            //: self.startPreview()
            self.readyAndWaitingGold()
        }
...
extension BagChatManager {
    ...
    private func domainFront(front: Bool) {
    ...
    private func enableeOff(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard main_versionFormat == true else {
            //: if isOpen {
            if isOpen {
...
extension BagChatManager {
    ...
    func supportSystemSettings() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(appDidEnterBackground(notif:)),
                                               selector: #selector(headCur(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(travelInward(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(briskSchedule(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(weekly(notif:)),
    ...
    @objc private func headCur(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func travelInward(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func briskSchedule(notif: Notification) {
    ...
    @objc func weekly(notif: Notification) {
...
extension BagChatManager {
    ...
    func readyAndWaitingGold(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            HoeThen.halfLine(msg: (userSpaceMessage.replacingOccurrences(of: "evaluation", with: "T") + String(mReplaceIdent)))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            HoeThen.halfLine(msg: (String(dream_teamName.suffix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (String(mExplorationContent.prefix(7)) + String(m_mineUrl)) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            detectBy(smallWindow: false)
    ...
    func detectBy(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard main_versionFormat == true else {
    ...
    private func cabooseLanguage() {
        ...
        params.sdkAppId = main_attributeStr
    ...
    private func stackView() {
        ...
        HoeThen.halfLine(msg: (String(mBullMsg.suffix(6)) + "频通话销毁." + String(appConnectId.suffix(4)) + k_labelTitle) + "\(infoModel?.logId ?? 0).")
...
extension BagChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        HoeThen.halfLine(msg: (mDownPath.replacingOccurrences(of: "pack", with: "T") + mFaceMessage + "通话错误." + String(appExpressionStr.prefix(4)) + "Code:") + "\(errCode)" + (String(k_byFormat.suffix(4)) + "rMsg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.changeManage(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        HoeThen.halfLine(msg: (String(notiReachFeedbackData) + String(kServerTopicIdent.suffix(3)) + app_editText.lowercased() + "gCode:") + "\(warningCode)" + (String(show_adminText) + String(show_todayQuoteMessage)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: noti_beginIdent, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        HoeThen.halfLine(msg: (mGivingValue.replacingOccurrences(of: "curve", with: "RT") + " \u{89c6}频通话进" + app_remarkUrl + " result:") + "\(result)" + (String(dream_entryWordMessage.suffix(4)) + "gId\u{ff1a}") + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            ...
            StackSocketManager.shared.nativeDown()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
