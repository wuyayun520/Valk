...
/*: "Speaker On" :*/
fileprivate let kRetPressName:[Character] = ["S","p","e","a","k","e","r"," ","O","n"]

/*: "Speaker Off" :*/
fileprivate let showStirAreaMessage:[Character] = ["S","p","e","a","k","e","r"," ","O","f","f"]

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let k_exhibitNowPubliclyName:String = "TRTC 音频compete flow"
fileprivate let dream_nearbyAlongContent:[Character] = ["\u{901a}","话"]
fileprivate let dream_resumeData:[Character] = ["销","毁","."," ","l","o","g","I","d","："]

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let dreamObjectActionId:[Character] = ["T","R","T","C"," ","音","频","通","话","错","误","."," ","e","r","r","C","o","d"]
fileprivate let mEffectiveUrl:[Character] = ["e",":"]

/*: , errMsg: :*/
fileprivate let userSendText:String = "portrait fail, err"
fileprivate let dreamHoldShowIdent:[Character] = ["M","s","g",":"]

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let user_resolutionValue:String = "TRTC \u{97f3}"
fileprivate let mGravityPath:String = "K收\u{5230}警告. w"
fileprivate let dreamTimeKey:String = "ARNI"

/*: , warningMsg: :*/
fileprivate let mainToStr:String = ", warnlocal engineering instance vessel"
fileprivate let main_drillStr:[Character] = ["i","n","g","M","s","g",":"]

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let mainSlightData:String = "then hair calculation publication bullTRTC 音频通"
fileprivate let show_rateMsg:[Character] = ["话","\u{8fdb}","\u{5165}","房","间","."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let show_selectedFormat:String = "retain gender tee act since, log"
fileprivate let kOccurValue:String = "ret work weight increasingId："

/*:  加入了房间" :*/
fileprivate let appCosText:[Character] = [" ","加","\u{5165}","了","房"]
fileprivate let userResortStatusId:String = "间"

/*:  离开了房间" :*/
fileprivate let app_applicationUrl:[Character] = [" ","离","开","\u{4e86}","房"]
fileprivate let show_sillyData:[Character] = ["间"]
...
private class FourthChatConfig: NSObject {
...
class EveryChatManager: NSObject {
    ...
    private var config = FourthChatConfig() // 音频渲染配置类
    //: private var logId = 0
    ...
    deinit {
        ...
        self.sodiumPyrophosphateFitLaborResources()
    }
}
...
extension EveryChatManager {
    ...
    func underHeart(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !connectionSet()
    ...
    func design(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard connectionSet() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(kRetPressName)).localized : (String(showStirAreaMessage)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        mouthCut(isHandsfree: isHandsfree)
    }
}
...
extension EveryChatManager {
    ...
    private func connectionSet() -> Bool {
...
extension EveryChatManager {
    ...
    func programExtended() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(notification_stopLocalAudio(notif:)),
                                               selector: #selector(advancedAudioStopCreateerLocal(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(delayNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(quantityerrupted(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(audioWithinTakerNotif(notif:)),
    ...
    @objc private func advancedAudioStopCreateerLocal(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func delayNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func quantityerrupted(notif: Notification) {
        ...
        DispatchQueue.main.async {
            ...
            if interruptStatus == AVAudioSession.InterruptionType.began {
                ...
                self.advancedAudioStopCreateerLocal(notif: nil)
    ...
    @objc func audioWithinTakerNotif(notif: Notification) {
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(kRetPressName)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            mouthCut(isHandsfree: self.config.isHandsfree)
...
extension EveryChatManager {
    ...
    private func mouthCut(isHandsfree: Bool) {
    ...
    private func universalApply() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        HoeThen.halfLine(msg: (String(k_exhibitNowPubliclyName.prefix(7)) + String(dream_nearbyAlongContent) + String(dream_resumeData)) + "\(logId).")
        //: guard self.config.isObject else { return }
        guard self.config.isObject else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            mouthCut(isHandsfree: true)
        }
...
extension EveryChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        HoeThen.halfLine(msg: (String(dreamObjectActionId) + String(mEffectiveUrl)) + "\(errCode)" + (String(userSendText.suffix(5)) + String(dreamHoldShowIdent)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        HoeThen.halfLine(msg: (user_resolutionValue + "频通话SD" + mGravityPath + dreamTimeKey.lowercased() + "ngCode:") + "\(warningCode)" + (String(mainToStr.prefix(6)) + String(main_drillStr)) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        HoeThen.halfLine(msg: (String(mainSlightData.suffix(8)) + String(show_rateMsg)) + "\(result)" + (String(show_selectedFormat.suffix(5)) + String(kOccurValue.suffix(3))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(appCosText) + userResortStatusId.capitalized))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(app_applicationUrl) + String(show_sillyData)))
    }
}
