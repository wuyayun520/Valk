...
fileprivate let user_completeIdent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#2DF2FF" :*/
fileprivate let dream_interestedMessage:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let noti_demonstrateKey:String = "given progressLive"

/*: "img_home_shadow_icon" :*/
fileprivate let app_picturePath:String = "img_hphone pretty maybe week huge"
fileprivate let mainSubmitStr:String = "adow_towards influential fit quietly"

/*: "get json error" :*/
fileprivate let k_popUrl:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r"]
fileprivate let userFeatureTitle:[Character] = ["o","r"]

/*: "#FF2348" :*/
fileprivate let app_merelyValue:String = "system bond sweep alive#FF2348"

/*: "#35C759" :*/
fileprivate let mReadPathLackId:String = "natural written increase depth#35C7"
fileprivate let main_previousId:String = "59"

/*: "mfBean" :*/
fileprivate let m_tabTitle:String = "eastern s discussion environmentmfBean"

/*: "points\n≈ $" :*/
fileprivate let notiAppMsg:String = "poicollect"
fileprivate let userReachPagePath:String = " $"
...
class DistantWrongView: BubbleMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= AppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(appSystemName - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                markMiniskirtPop()
    ...
    override init(frame: CGRect) {
        ...
        if Double(AppManager.share.loginUserMode.mf_bean) > 0 {
            ...
            standBackUpwardsLocalLegume(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_completeIdent.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: GoddamnThen = {
        ...
        let image = UIImage.fillAgain(color: UIColor(hex: (String(dream_interestedMessage)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.license(), for: .normal)
        img.setImage(image.license(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(noti_demonstrateKey.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.verbalDescription(name: (String(app_picturePath.prefix(5)) + "ome_sh" + String(mainSubmitStr.prefix(5)) + "icon"))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(k_popUrl) + String(userFeatureTitle)))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(app_merelyValue.suffix(7))))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(mReadPathLackId.suffix(5)) + main_previousId.capitalized))
...
extension DistantWrongView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func host() -> DistantWrongView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = DistantWrongView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = StackSocketManager.shared.liveMiniPosition
    ...
    func markMiniskirtPop() {
        //: liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        liveStatusImgV.isHidden = !GuideUnitReactiveCompatible.commonFragment().isLive
...
extension DistantWrongView {
    ...
    private func notSign() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(calledManage(notification:)),
    ...
    @objc private func calledManage(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(m_tabTitle.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        standBackUpwardsLocalLegume(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func standBackUpwardsLocalLegume(mfBean: Double) {
        ...
        mfBeanLB.text = "\(Int(nowBean))\((notiAppMsg.replacingOccurrences(of: "collect", with: "n") + "ts\n\u{2248}" + userReachPagePath.capitalized).localized) \(divisor)"
