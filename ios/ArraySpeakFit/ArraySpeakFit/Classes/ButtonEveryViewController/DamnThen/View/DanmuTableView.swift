...
fileprivate let mWantClearRemoteContent:[UInt8] = [0x8e,0x93,0x8e,0x99,0x4d,0x88,0x94,0x89,0x8a,0x97,0x5f,0x4e,0x45,0x8d,0x86,0x98,0x45,0x93,0x94,0x99,0x45,0x87,0x8a,0x8a,0x93,0x45,0x8e,0x92,0x95,0x91,0x8a,0x92,0x8a,0x93,0x99,0x8a,0x89]

fileprivate func criticizeIgnore(decrease num: UInt8) -> UInt8 {
...
enum ImmediatelyBinaryMagnitude: Int {
    //: case TimeGap = 0
...
@objc protocol UnitObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func uncertainness()

    //: @objc optional func endScroll()
    @objc optional func coverOk()
}

//: class TalkingDanmuMsgListTableView: UIView {
class DanmuTableView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mWantClearRemoteContent.map{criticizeIgnore(decrease: $0)}, encoding: .utf8)!)
    }
...
extension DanmuTableView {
    ...
    func capacityClean() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(towardReading), userInfo: nil, repeats: true)
    }
...
extension DanmuTableView {
    ...
    func joinNor(msgModel: PhysicalModelType?) {
    ...
    func role() {
        ...
        if AllHeight > mGreetIdent {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < mGreetIdent {
                ...
                self.msgTableView.height = mGreetIdent
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = mGreetIdent - AllHeight
        ...
        steep(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func steep(animated: Bool) {
...
extension DanmuTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func that(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.coverOk?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(m_plainName)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = LicenseFaceBassCell.spectrum(tableView: tableView, msg: msgModel, indexPath: indexPath)
