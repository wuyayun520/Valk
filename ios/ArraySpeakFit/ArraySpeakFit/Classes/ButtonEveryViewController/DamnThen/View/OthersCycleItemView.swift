...
/*: "#ffffff" :*/
fileprivate let mNarrowMsg:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let user_saltMutualName:[Character] = ["/","d","i","s","t","/","l","o","u","n","g","e","P","l","u","s","/"]
fileprivate let m_progressiveValue:String = "indhard"
fileprivate let dreamPremiumMsg:[Character] = ["x",".","h","t","m","l"]

/*: "https://" :*/
fileprivate let kFormerMessage:String = "end"
fileprivate let mBeatChaseName:[Character] = ["t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let notiInvestLightSweepContent:String = "http://may resume"
...
class OthersCycleItemView: UIView {
    ...
    let Broadcast_TextColor = (String(mNarrowMsg))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .equitySize(fontSize: 12)
    ...
    convenience init(model: AlongsideTransformable?) {
        //: self.init()
        self.init()
    ...
    private func familyWith() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.tipIndex(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func beScene(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.beyondFore(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.minAllow(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: AlongsideTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            familyWith()
    ...
    var totalTime: CGFloat {
        ...
        return (kGiftIdent + widthValue) * Broadcast_SpendTime / kGiftIdent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / kGiftIdent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / kGiftIdent
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(generateDoing(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.equitySize(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension OthersCycleItemView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func generateDoing(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: PipSqueakReactiveCompatible.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            PipSqueakReactiveCompatible.share.balanceConvertSeek(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: PipSqueakReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                PipSqueakReactiveCompatible.share.reOff(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(user_saltMutualName) + m_progressiveValue.replacingOccurrences(of: "hard", with: "e") + String(dreamPremiumMsg))) {
                ...
                let httpUrl = httpsUrl.replacingOccurrences(of: (kFormerMessage.replacingOccurrences(of: "end", with: "ht") + String(mBeatChaseName)), with: (String(notiInvestLightSweepContent.prefix(7))))
                //: PipSqueakReactiveCompatible.share.func__pushToWebVC(urlStr: httpUrl)
                PipSqueakReactiveCompatible.share.pokeAt(urlStr: httpUrl)
            }
