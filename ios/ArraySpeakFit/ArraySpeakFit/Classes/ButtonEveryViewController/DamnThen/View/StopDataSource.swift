...
fileprivate let userChaseIdent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let notiProposalFormat:[Character] = ["O","n","l","i","n","e"," ","U"]
fileprivate let k_noseName:[Character] = ["s","e","r","s"]
...
class StopDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userChaseIdent.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cycleForthDoingSnap), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(notiProposalFormat) + String(k_noseName)).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension StopDataSource {
    ...
    private func lateTo() {
        ...
        DemandAudienceManager().imageCrop(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                if let datas = Array<CycleTransformable>.deserialize(from: array as? Array) {
...
extension StopDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.seatAndOrderedSeries(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension StopDataSource {
    ...
    @objc private func cycleForthDoingSnap() {
        //: dismiss()
        labBench()
    }
    ...
    func boxDoing(chatGroupId: String) {
