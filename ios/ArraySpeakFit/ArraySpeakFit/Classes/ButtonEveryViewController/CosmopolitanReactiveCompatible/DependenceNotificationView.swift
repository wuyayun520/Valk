...
fileprivate let notiWeddingName:[UInt8] = [0xcc,0xcb,0xcc,0xd1,0x8d,0xc6,0xca,0xc1,0xc0,0xd7,0x9f,0x8c,0x85,0xcd,0xc4,0xd6,0x85,0xcb,0xca,0xd1,0x85,0xc7,0xc0,0xc0,0xcb,0x85,0xcc,0xc8,0xd5,0xc9,0xc0,0xc8,0xc0,0xcb,0xd1,0xc0,0xc1]

private func damageStage(clearly num: UInt8) -> UInt8 {
    return num ^ 165
}

/*: "Reply" :*/
fileprivate let dreamRegionId:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let dreamReachValue:String = "#F95151tolerance contrast clean that"
...
// __CLOSE_PRINT__
//
//  DependenceNotificationView.swift
//  AbroadTalking
//
...
let dreamMaleIdent = (dreamNoSampleValue + 80.0)
//: class TalkingMessageNotificationView: UIView {
class DependenceNotificationView: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: SayPreviouslyReactiveCompatible) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: DependenceNotificationView) -> Void)? // 完成移除视图Block
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiWeddingName.map{damageStage(clearly: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .concern(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .concern(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(dreamRegionId)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(dreamReachValue.prefix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension DependenceNotificationView {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func pursuingShow(_ model: SayPreviouslyReactiveCompatible?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    ...
                    self.resignAgree(needAlpha: false)
                }
            }
        }
    }
    ...
    func resignAgree(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -dreamMaleIdent
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func towardCut() {
        ...
        if currModel.chatType == .service {
            ...
            //: PipSqueakReactiveCompatible.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            PipSqueakReactiveCompatible.share.vidScan(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func field(gesture: UISwipeGestureRecognizer) {
...
extension DependenceNotificationView {
    ...
    func doBy() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.beyondFore(urlStr: userInfo.headPic, placeImg: UIImage.salt(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.setFromImaginationImage(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func betwixt() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(towardCut))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(field))
        //: swipe.direction = .up
        swipe.direction = .up
