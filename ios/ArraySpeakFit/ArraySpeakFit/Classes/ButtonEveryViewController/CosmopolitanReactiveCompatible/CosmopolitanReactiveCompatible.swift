...
/*: "rYMsgType" :*/
fileprivate let show_launchScreenPath:String = "burn arrival costrYMsgTy"
fileprivate let mVeryTitle:String = "hardwaree"

/*: "GJ:CallCustom" :*/
fileprivate let k_frameId:String = "GJ:Calike sodium him tool bottom"
fileprivate let showHangMsg:String = "display format behind off betterllCustom"
...
// __CLOSE_PRINT__
//
//  CosmopolitanReactiveCompatible.swift
//  AbroadTalking
//
...
class CosmopolitanReactiveCompatible: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: DependenceNotificationView? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: DependenceNotificationView? // 旧的View
    ...
    static var shared = CosmopolitanReactiveCompatible()
    ...
    @objc func indue() {
        ...
        if self.preView != nil {
            ...
            self.preView?.resignAgree()
        }
        //: if self.showView != nil {
        if self.showView != nil {
            //: self.showView?.dismiss()
            self.showView?.resignAgree()
        }
    ...
    private lazy var msgArr: [SayPreviouslyReactiveCompatible] = //: return [SayPreviouslyReactiveCompatible]()
        .init()
    //: }()
}
...
extension CosmopolitanReactiveCompatible {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func overMutual(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard random() == true || bmi() == true else { return }
        //: guard msg.userID != ImmediatelyThen.getCustomerServiceID() && msg.userID != ImmediatelyThen.getXiaoMiID() else {
        guard msg.userID != ImmediatelyThen.femaleId(), msg.userID != ImmediatelyThen.polytetrafluoroethylene() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = GrapplingIronReactiveCompatible.reduce(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(show_launchScreenPath.suffix(7)) + mVeryTitle.replacingOccurrences(of: "hardware", with: "p"))] as? String == (String(k_frameId.prefix(5)) + String(showHangMsg.suffix(8))) {
            //: return
            return
        }

        // 2. V2TIMMessage => SayPreviouslyReactiveCompatible
        ...
        //: if let userInfo: RelatedTransformable = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: RelatedTransformable = StateThen.goldId(targetId: model.targetId) {
    ...
    private func nextView() {
        ...
        let notifView = DependenceNotificationView(frame: .zero)
        //: notifView.show(firstModel)
        notifView.pursuingShow(firstModel)
    ...
    private func random() -> Bool {
        ...
        guard let currentVC = decideBroad() else { return false }
        //: if currentVC.isKind(of: TalkingVideoChatViewController.self) ||
        if currentVC.isKind(of: FourthViewController.self) ||
            //: currentVC.isKind(of: TalkingVoiceChatViewController.self) {
            currentVC.isKind(of: StackErrorDelegate.self)
        {
            //: return true
            return true
        }
